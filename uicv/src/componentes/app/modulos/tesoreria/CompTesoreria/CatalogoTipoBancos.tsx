...
import * as Funciones from './CatalogoTipoBancos/Funciones'
...
import { CForm } from './CatalogoTipoBancos/CForm'
...
const CatalogoBancoBancos = (props: CatalogosType) => {
    ...
    const Columns = React.useMemo(() => {
        let colRet: IDataTableColumn[] =
            [
                { name: 'Id', selector: 'tipoBancoID', sortable: true, },
    ...
    const cbAgregar = (item: any) => {
        toast.success('El tipo de banco se agrego correctamente')
        setState({
    ...
    const cbActualizar = (item: any) => {
        ...
        setState(state => ({
            ...state, Datos: state.Datos.map(Dato => Dato.tipoBancoID === item.tipoBancoID ? item : Dato),
            Form: {
                ...state.Form, Mostrar: false,
                Datos: {
                    tipoBancoID: 0,
    ...
    return (
        <div className="row">
            <div className="col-12">
                <Card Title="Catalogo Tipo Bancos">
                    <Card.Body>
                        <Card.Body.Content>
                            ...
                            {!state.Cargando && !state.Error &&
                                <div>
                                    <DataTable
                                            ...
                                            <div className="row">
                                                <div className="col-sm-12">
                                                    <div className="input-group mb-3">
                                                        <input type="text" className="form-control" placeholder="Buscar Tipo Banco" value={state.Filtro} onChange={e => setState(s => ({ ...s, Filtro: e.target.value }))} />
                                                        <span className="input-group-text"><FaSearch /> </span>
                                                        <button className="btn btn-outline-secondary" type="button"
                                                            onClick={() => setState({ ...state, Form: { Mostrar: true, Datos: DatosDefecto, Id: undefined } })}
                                                        ><FaPlus /></button>
                                                        <button className="btn btn-outline-secondary" type="button" onClick={() => FNGetDatos()}><FiRefreshCcw /></button>
                                                    </div>
                                                </div>
                                            </div>
                                        }
                                        ...
                                        noHeader
                                        responsive
                                        keyField={"tipoBancoID"}
                                        defaultSortField={"tipoBancoID"}
                                        columns={Columns}
                                    />
                                    <ModalWin open={state.Form.Mostrar}>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>
                                                ...
                                                {state.Form.Id ? "Editar Tipo de Banco" : "Agregar Tipo de Banco"}

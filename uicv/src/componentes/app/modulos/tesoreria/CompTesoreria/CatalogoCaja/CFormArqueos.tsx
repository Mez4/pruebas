...
export const CFormArqueos = (props: CFormType) => {
  const DatosDefecto = {
    NombreMoneda: "",
    TipoCambio: 0,
    Fecha: new Date(),
      ...
      cell: (propss) => (
        <button
          ...
          onClick={() => {
            ...
            ImprimirCorte(propss.CajaID, propss.ArqueoID);
            /*  setState(s => ({
                             ...s, Form: {
  ...
  const FNGetPolizas = (
    ...
    setLoading(true);
    Funciones.FNGetArqueos(props.Seguridad, cajaId, fechaInicio, fechaFin)
      .then((respuesta: any) => {
  ...
  const ImprimirCorte = (cajaId?: number, arqueoId?: number) => {
    setLoading(true);

    Funciones.ImprimirCorte(props.Seguridad, cajaId, arqueoId)
      .then((respuesta: any) => {

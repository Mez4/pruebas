...
import { Card, Spinner, CustomFieldText, ActionSelect, ActionCreatableSelect, ActionFieldText2, ActionAsyncSelect, Carrusel, CustomFieldDatePicker, ImgViewer } from '../../../../../global'
...
import { FaShoppingCart, FaCloudDownloadAlt, FaAddressCard, FaEye } from 'react-icons/fa'
...
import { fetchExcepcionHuella, fetchHuellaCliente, fetchHuellaSucursal, fetchLectorHuella } from '../../../personas/CompAdministracion/Funciones'
...
type CFormType = {
    ...
    fnGetUltimoBeneficiario(ClienteID: number, Form?: any): any,    
    ...
    optFolios: { value: number, label: string, estatus: string, valedigital: boolean }[],
    isUpdate: boolean,
    DatosCliente: any,
...
export const CForm = //memo(
    (props: CFormType) => {
        const [ArticulosCarrito, setArticulosCarrito] = useState([])
        const [Distribuidor, setDistribuidor] = useState(0)
        ...
        // const [ArticulosCarrito, setArticulosCarrito] = useState<string[]>([]);
        ...
        const refCapital = useRef<Select>(null)
        ...
        const clearFormByLevel = (level: number) => {
            if (level === 0) {
                ...
                refSerie.current?.select.setValue({ value: '0', label: '' }, "deselect-option")
            }
            if (level === 0 || level === 1) {
                setFormValues(s => ({ ...s, optCapital: [], optFolios: [], optPlazos: [] }))
                refDistribuidor.current?.select.setValue({ value: '0', label: '' }, "deselect-option")
                refTipoDesembolso.current?.select.setValue({ value: '0', label: '' }, "deselect-option")
                refParentescos.current?.select.setValue({ value: '0', label: '' }, "deselect-option")
        ...
        const validarDistribuidor = (value: any) => {
            console.table('validarDistribuidor', value)
        ...
        const getserievalera = (value: any) => {
            // console.log('VALOOOOOOOOOr',value.SucursalID)
            FnPersona.FNGeValeraSocia(props.oidc,value.SucursalID )
            .then((respuesta: any) => {
                    console.log('AQUIDEBEVENIR LA VALERA',respuesta)
                    if (respuesta ) {
                        toast.info(`La serie de la valera de esta socia es: ${respuesta.serie}`)
                    }
        ...
        //Validar Cliente
        const validarCliente = (value: any) => {
            FnPersona.FNGetCliente(props.oidc, {
                ClienteId: value
            })
                .then((respuesta: any) => {
                    if (!respuesta.CanjeaVale) {
                        toast.info(`El cliente ${value} está bloqueado temporalmente`);
                        props.fnGetUltimoBeneficiario(0);
                    } else {
                        console.log(formValues.DistribuidorId)
                        props.fnGetUltimoBeneficiario(respuesta.ClienteID);
                    }
                })
                .catch(() => {
        ...
        const cbCliente = (value: any) => { 
            ...
            .then((respuesta: any)=>{
                if (!respuesta.CanjeaVale) {
                    ...
                    props.fnGetUltimoBeneficiario(respuesta.ClienteID, formValues);
                    console.log('respuesta', Distribuidor)                             
                }
            })
            .catch(() => {
                toast.error("Ha ocurrido un error al obtener al cliente")
            })
            //setFormValues(s => ({ ...s, DistribuidorId: Number(Distribuidor) }))
        }
        ...
        useEffect(() => {
            console.log('DatosDist: ', props.DatosDistribuidor)
            ...
            if (!props.optFolios.length && !props.isUpdate) {
                MySwal.fire({
                    ...
                    html:
                        <div className="text-center">
                            El folio: {Estado.values.Folio} no pertenece a la {DescripcionDistribuidor(1)} con el id: {Estado.values.DistribuidorId} para la serie: {Estado.values.SerieId}
                        </div>,
                ...
                if ((shopInfo.totalPrice <= formValues.TabTiendita.importeMaxCanje)) {
                    ...
                    if (props.Id! > 0) {
                        ...
                        Funciones.FNUpdate(props.oidc, {
                            ...
                            .then((respuesta: any) => {
                                ...
                                if (respuesta.regresa === 1) {
                                    toast.success(`Se modificó el crédito con el N° ${respuesta.CreditoId}`)
                        ...
                        Funciones.FNCheckBox(props.oidc, Estado.values.CajaID)
                            .then((res: any) => {
                                // console.log(res);
                                        ...
                                        .then((respuesta: any) => {
                                            if (respuesta.status || respuesta.regresa === 1) {
                                                ...
                                                if (respuesta.TipoDesembolsoID != 7) {
                                                    ...
                                                    Funciones.FNPdf2(props.oidc, { CreditoId: respuesta.CreditoId, personasDatosBancariosID: props.initialValues.personasDatosBancariosID })
                                                        .then((respuesta: any) => {
                    ...
                    toast.error(`El límite de compra en la tiendita para este vale es de $${formValues.TabTiendita.importeMaxCanje}`)
                }
            }
        ...
        useEffect(() => {
            setFormValues(s => ({ ...s, optDistribuidores: props.optDistribuidores }))
        ...
        useEffect(() => {
            if (props.optTiposDesembolso.length > 1)
                setFormValues(s => ({ ...s, optTiposDesembolso: props.optTiposDesembolso }))
        ...
        useEffect(() => {
            if (props.initialValues.Capital > 0) {
                ...
                const capital: any = refCapital.current?.select
                capital.select.setValue({ value: props.initialValues.Capital, label: props.initialValues.Capital }, "select-option")
        ...
        useEffect(() => {
            if (props.initialValues.ClienteId > 0) {
                setFormValues(s => ({ ...s, ClienteId: props.initialValues.ClienteId }))
            }
        ...
        const imageViewRef = useRef<HTMLDivElement>(null);//Referecia para la vista previa de la imagen
        const drop_area = useRef<HTMLDivElement>(null);//Referencia para el area de arrastrar y soltar

        const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            try {
                if (!event.target.files || event.target.files.length === 0) {
                    toast.error("No se ah seleccionado algun archivo");
                    throw new Error("No se ah seleccionado algun archivo");
                ...
                if (imageViewRef.current) {
                    ...
                    //Elminar el texto de la imagen
                    imageViewRef.current.innerHTML = '';
                    //Ajustar la iamgen al contenedor
                    imageViewRef.current.style.backgroundSize = 'cover';
                    //Ajustar la imagen al centro
                    imageViewRef.current.style.backgroundPosition = 'center';
                    //Imagen para que no se repita
                    imageViewRef.current.style.backgroundRepeat = 'no-repeat';
                }
            } catch (error) {
        ...
        const handleDrop = (event: DragEvent) => {
            ...
            if (event.dataTransfer && event.dataTransfer.files.length > 0) {
                ...
                if (imageViewRef.current) {
                    ...
                    imageViewRef.current.style.backgroundSize = 'cover';
                    imageViewRef.current.style.backgroundPosition = 'center';
                    imageViewRef.current.style.width = '100%';
                    imageViewRef.current.style.height = '100%';
                    imageViewRef.current.style.backgroundRepeat = 'no-repeat';
                    imageViewRef.current.style.objectFit = 'cover';
                }
            }
        };
        ...
        useEffect(() => {
            if (props.initialValues.personasDatosBancariosID > 0) {
                setFormValues(s => ({ ...s, personasDatosBancariosID: props.initialValues.personasDatosBancariosID }))
            }
        ...
        const FNGetPersona = (PersonaID: number, e:any) =>{
             ...
             .then((respuesta:any)=>{
               ...
               let ProductoID = props.ui.Producto?.ProductoID
               console.log("BASEDATOS",props.ui.DireccionIP)
              let BD = props.ui.DireccionIP
               requestFingerprint(e,respuesta.CURP,ProductoID,userID, BD)
             })
            
          }
          const btnHuella = React.useRef<any>();
          const btnSubmit = React.useRef<any>()
          // WEB SOCKET HUELLAS
          ...
          function connectWebSocket() { 
            ...
            socket.addEventListener("message", event => {
              ...
              console.log("Contenido RESPUESTA",parsedData) 
          ...
          const requestFingerprint = (e, Curp, producto, usuarioid, bd) => {        
            console.log("This props",props)
            e.preventDefault()
          ...
          let Bdselec = 0
              ...
              link.href = `cv://registrarhuella?productoid=${producto}&curp=${Curp}&usuarioid=${usuarioid}&bd=${Bdselec}`;
          ...
          useEffect(() => {
            ...
            fetchHuellaSucursal(props.oidc, formValues.SucursalId)
              .then((respuesta: any) => {
                //setAllowHuellas(respuesta);
                if(respuesta){
                    fetchExcepcionHuella(props.oidc, formValues.ClienteId)
                    .then((respuesta2: any) => {
            ...
            fetchHuellaCliente(props.oidc, formValues.ClienteId)
            .then((respuesta: any) => {
                console.log("Cliente", formValues.ClienteId, respuesta)
                if(respuesta){
          ...
          }, [props.oidc, formValues.ClienteId]); 
            ...
            <>
                <Formik
                    ...
                    validationSchema={Yup.object().shape({
                        ...
                        Folio: Yup.number().required("Ingrese el folio del vale").moreThan(0, 'Ingrese el folio del vale'),
                        SerieId: Yup.number().required("Seleccione la serie del vale").moreThan(0, 'Seleccione la serie del vale'),
                        Capital: Yup.object().shape({
                            label: Yup.string().required(),
                            value: Yup.string().required(),
                        }).required("Ingrese el capital").nullable(), //Yup.number().required("Ingrese el capital").moreThan(0, 'Ingrese el capital'),
                        ...
                        FechaExpedicion: Yup.date().min(moment().add(-30, 'd').toDate()).required("La fecha de expedicion supera los 30 dias"),
                        personasDatosBancariosID: Yup.number().when('RequiereDatosBancarios', {
                            is: true,
                            then: Yup.number().required(`Seleccione la cuenta o la CLABE del cliente`).moreThan(0, `Seleccione la cuenta o la CLABE del cliente`)
                        }),
                        NombreBeneficiario: !props.isUpdate ? Yup.string().required("Ingrese el nombre del beneficiario") : Yup.string().nullable(true),
                        ApellidoPaternoBeneficiario: !props.isUpdate ? Yup.string().required("Ingrese el apellido paterno del beneficiario") : Yup.string().nullable(true),
                        ApellidoMaternoBeneficiario: !props.isUpdate ? Yup.string().required("Ingrese el apellido materno del beneficiario") : Yup.string().nullable(true),
                        ParentescoBeneficiario: !props.isUpdate ? Yup.number().required("Seleccione el parentesco con el beneficiario") : Yup.number().nullable(),
                        FechaNacimientoBeneficiario: !props.isUpdate ? Yup.string().required().test("FechaNacimientoBeneficiario", "El beneficiario debe ser mayor de edad", (value: any) => moment().diff(moment(value), "years") >= 18) : Yup.string().nullable(true)
                    })}
                    onReset={(values: any) => {
                        clearFormByLevel(0)
                    }}
                    onSubmit={(values: any, { resetForm }) => {
                        ...
                        Estado.values.ApellidoPaternoBeneficiario = values.ApellidoPaternoBeneficiario;
                        Estado.values.ApellidoMaternoBeneficiario = values.ApellidoMaternoBeneficiario;
                        Estado.values.ParentescoBeneficiario = ParentescoBeneficiarioValues[values.ParentescoBeneficiario - 1].label;
                        Estado.values.FechaNacimientoBeneficiario = values.FechaNacimientoBeneficiario;
                        ...
                        if (!props.optFolios.length && !props.isUpdate) {
                            MySwal.fire({
                                ...
                                html:
                                    <div className="text-center">
                                        El folio: {values.Folio} no pertenece a la {DescripcionDistribuidor(1)} con el id: {values.DistribuidorId} para la serie: {values.SerieId}
                                    </div>,
                                showCloseButton: false,
                                showCancelButton: true,
                                showConfirmButton: false,
                            ...
                            if(huellaConfirmada || !allowHuellas){
                                if ((shopInfo.totalPrice <= formValues.TabTiendita.importeMaxCanje)) {
                                    ...
                                    if (props.Id! > 0) {
                                        ...
                                        ArticulosCarrito.map((Dato: any) => { Dato.id_estructura === 472 || Dato.id_estructura === 471 || Dato.id_estructura === 473 ? EsArticuloEspecial = 1 : EsArticuloEspecial = 0 })
                                        ...
                                        if (EsArticuloEspecial === 0) {
                                            ...
                                            Funciones.FNCheckBox(props.oidc, values.CajaID)
                                                .then((res: any) => {
                                                    // console.log(res);
                                                    ...
                                                    if (!res.Cerrada) {
                                                        ...
                                                        Funciones.FNAdd(props.oidc, {
                                                            ...
                                                            articles: ArticulosCarrito,
                                                            TipoCanje: 1,
                                                            ParentescoBeneficiario: ParentescoBeneficiarioValues[values.ParentescoBeneficiario - 1].label,
                                                            Sucursal: SucTienda,
                                                            Validado: Estado.Validado
                                                            // ArticulosCarrito: ArticulosCarrito
                                                        })
                                ...
                                toast.error("No ha escaneado las huellas")
                            }
                        ...
                        <Form>
                            ...
                            <div className="columns is-desktop is-tablet">
                                <div className="column is-12-mobile is-12-tablet is-4-desktop">
                                    <ActionSelect
                                        disabled={props.isUpdate || loading} //props.isUpdate ||
                                        ...
                                        accion3={getserievalera}
                                    />
                                </div>
                                <div className="column is-12-mobile is-half-tablet is-2-desktop">
                                    <br className="is-hidden-touch" />
                                    {/* <button title='Ver Firma' type="button" disabled={false} className="ms-2 btn btn-primary waves-effect waves-light" */}
                                    <button title='Ver Firma' type="button" disabled={loading || formValues.DistribuidorId == 0 || documentoID == 0 || !documentoID} className="ms-2 btn btn-primary waves-effect waves-light"
                                        onClick={() => setVerFirma(true)}>
                                        Firma {`${DescripcionDistribuidor(1)}`}&nbsp;<FaAddressCard />
                                    </button>
                                </div>
                                <div className="column is-12-mobile is-12-tablet is-4-desktop">
                                    {props.isUpdate &&
                                ...
                                <div className="column is-12-mobile is-half-tablet is-2-desktop">
                                    <br className="is-hidden-touch" />
                                    <button title='Cliente' type="button" disabled={loading || formValues.ClienteId == 0} className="ms-2 btn btn-primary waves-effect waves-light" onClick={async () => {
                                        DefinirEstado(e => ({ ...e, Datos: { Persona: undefined, Direcciones: [], Empleos: [], ContCreditos: [], Creditos: undefined }, Cargando: true, Error: false }))
                                        let resultado = await FnPersona.FNObtenerPersona(props.oidc, formValues.ClienteId)
                                        DefinirEstado(e => ({
                                            ...e,
                                            Datos: {
                                        ...
                                        Información Cliente&nbsp;<FaAddressCard />
                                    </button>
                                </div>
                            </div>
                            <div className="columns is-desktop is-tablet">
                                ...
                                <div className="column is-12-mobile is-12-tablet is-one-quarter-desktop">
                                    <ActionFieldText2
                                        ...
                                        onChange={cbFolio}
                                    />
                                    {(!props.optFolios.length && !props.isUpdate && !!values.Folio) &&
                                        <div className="text-danger">{`El folio ${values.Folio} no pertenece a la ${DescripcionDistribuidor(1)} seleccionada`}</div>
                                    }
                                    {(props.optFolios[0]?.valedigital && !props.isUpdate && !!values.Folio) &&
                                        <div className="text-danger">{`El folio ${values.Folio} es digital`}</div>
                                    }
                                    {(props.optFolios[0]?.estatus == 'C' && !props.isUpdate && !!values.Folio) &&
                            ...
                            <div className="columns is-desktop is-tablet">
                                ...
                                <div className="column is-12-mobile is-12-tablet is-one-third-desktop">
                                    <br />
                                    <ControlDatosBancarios name={`personasDatosBancariosID`} RequiereDatosBancarios={props.DatosTipoDesembolso.RequiereDatosBancarios ?? false} ClienteId={values.ClienteId} valor={formValues.personasDatosBancariosID} fnOpen={() => setShowDatosBancarios(true)} />
                                </div>
                            </div>
                            <div className="columns is-desktop is-tablet">
                                <div className="column is-two-thirds">

                                    {
                                        ...
                                        <div>
                                            ...
                                            <button title='Tiendita' type="button" disabled={loading || formValues.DistribuidorId == 0 || (formValues.TabTiendita.importeMaxCanje == 0 || props.ProdTiendita == 0)
                                                || (props.DatosTipoDesembolso.TipoDesembolso == 'SPEI POR STP' || props.DatosTipoDesembolso.TipoDesembolso == 'ODP' || props.DatosTipoDesembolso.TipoDesembolso == 'FOLIO DIGITAL' || props.DatosTipoDesembolso.TipoDesembolso == 'FOLIO CONFIASHOP' || props.DatosTipoDesembolso.TipoDesembolso == 'MERCANCIA CONFIASHOP' || props.DatosTipoDesembolso.TipoDesembolso == 'Finiquito en Efectivo' || props.DatosTipoDesembolso.TipoDesembolso == 'PPI' || props.DatosTipoDesembolso.TipoDesembolso == 'Proteccion de Saldos')
                                                || props.isUpdate ? true : false} className="ms-2 btn btn-primary waves-effect waves-light" onClick={() => {
                                                    setShowStore(true)
                                                    clearFormByLevel(4)
                                                }}>
                                            ...
                                            <button ref={btnHuella} data-tip className="ms-2 btn btn-primary waves-effect waves-light" type={"button"} hidden={!allowHuellas} disabled={loading ||formValues.ClienteId==0}
                                            onClick={(e) => {
                                                let cliente = formValues.ClienteId
                                                FNGetPersona(cliente,e)
                                ...
                                <div className="column is-one-third">
                                    <h1 className="title is-3">{`TOTAL: $${total.toLocaleString("en")}`}</h1>
                                </div>
                            </div>
                            {
                                ...
                                <div className="columns is-desktop is-tablet">
                                    <div className="column is-6">
                                        ...
                                        <div className="box" style={{ maxHeight: '370px', overflowY: 'auto' }}>
                                            <h1 className="title is-5">Elementos de Tiendita en el carrito: <span>{ArticulosCarrito.length}</span><br />{ArticulosCarrito.length > 2 ? '(Desplazate hacia abajo)' : ''}</h1>
                                            {ArticulosCarrito.map((item: any) => (
                                                <div>
                                                    ...
                                                    <div className="columns is-desktop is-tablet">
                                                        ...
                                                        <label className="form-label mb-0">{`Precio: `}{item.descuento > 0 ? (
                                                              <>
                                                                <del style={{ position: 'relative', display: 'inline-block',color:'#c2c0c0' }}>{FormateoDinero.format(item.precioOrg)} </del> <span style={{color:'red'}}>{FormateoDinero.format(item.price)}</span>
                                                              </>
                                                            ) : (
                                                              `${FormateoDinero.format(item.price)}`
                                                            )}</label><br />
                                                            <label className="form-label mb-0">{`Cantidad: ${item.qty}`}</label><br />
                                                            <label className="form-label mb-0">{`Importe: `}{item.descuento > 0 ? (
                                                              <>
                                                                <del style={{ position: 'relative', display: 'inline-block',color:'#c2c0c0' }}>{FormateoDinero.format(item.precioOrg * item.qty)} </del> <span style={{color:'red'}}>{FormateoDinero.format(item.price * item.qty)}</span>
                                                              </>
                                                            ) : (
                                                              `${FormateoDinero.format(item.price * item.qty)}`
                                                            )}</label><br />
                                                            <label className="form-label mb-0">{`SKU: ${item.id}`}</label><br />
                                                            <label className="form-label mb-0">{`Código Barras: ${item.codigo}`}</label>
                                                            <br />
                                                        </div>
                                                        <div className="column is-4">
                                                            <ImgViewer imgSrc={item.imagen} noToolbar={true} zIndex={1500} maxWidth={100} maxHeight={300} />
                                                        </div>
                                                    </div>
                                                    <hr />
                                                </div>
                                    ...
                                    <div className="column">
                                        <div className="box">
                                        <p className="title is-5">Total: {shopInfo.totalPrice != shopInfo.totalPriceOrg ? (
                                                              <>
                                                                <del style={{ position: 'relative', display: 'inline-block',color:'#c2c0c0' }}>{FormateoDinero.format(shopInfo.totalPriceOrg)} </del> <span style={{color:'red'}}>{FormateoDinero.format(shopInfo.totalPrice)}</span>
                                                              </>
                                                            ) : (
                            ...
                            {loading && <Spinner />}
                            {
                                ...
                                <div className="text-end">
                                    ...
                                    <button title='Cancelar' type="reset" disabled={props.isUpdate} className="btn btn-danger waves-effect waves-light" onClick={props.fnCancelar}>
                                        Cancelar
                                    </button>
                                    <button type="submit" className="ms-2 btn btn-success waves-effect waves-light" ref={btnSubmit} disabled={allowHuellas}>
                                        Generar Crédito
                                    </button>
                ...
                <ModalWin open={resCredito}>
                    ...
                    <ModalWin.Body>
                        ...
                        {ArticulosCarrito.length > 0 &&
                            <div className="btn-group" role="group" aria-label="Basic example">
                                ...
                                <button
                                    ...
                                    className={`btn btn - ${ComprarTienda ? 'warning' : 'primary'} d-none`}
                                    type="button"
                                    disabled={CanjearVale || ComprarTienda || errorVale || (creditoTienda > 0)}
                                    onClick={() => {
                                        setComprarTienda(true)
                                        var detalle = ArticulosCarrito.map((valor: any) => {
                                            var obj = { id_sku: valor.id, cantidad: valor.qty, estructura: valor.estructura };
                                            return obj
                                        });
                                        ...
                                        let JsonTda = {
                                            ...
                                            , sucursal: SucTienda.id_origen //props.initialValues.SucursalId
                                            , tipo_usuario1: "4"
                                            , id_usuario: formValues.DistribuidorId
                                            , sistema1: sistema //props.Sistema
                                            , id_forma_pago: 10
                        ...
                        <div className="text-end mt-5">
                            {/* <button type="button" disabled={loading || (creditoVale > 0 && creditoTienda === 0 && ArticulosCarrito.length > 0)} className="ms-2 btn btn-danger waves-effect waves-light" onClick={() => { */}
                            ...
                            {/* <button type="button" disabled={loading || (creditoVale > 0 && creditoTienda === 0 && ArticulosCarrito.length > 0)} className="ms-2 btn btn-primary waves-effect waves-light" */}
                            <button type="button" className="ms-2 btn btn-primary waves-effect waves-light"
                                ...
                                Descargar C. Responsiva&nbsp;<FaCloudDownloadAlt />
                            </button>
                            <button type="button" className="ms-2 btn btn-primary waves-effect waves-light"
...

                {
                    ...
                    <ModalWin open={ShowStore} large center scrollable>
                        <ModalWin.Header>
                            ...
                            <button type="button" className="delete" onClick={() => setShowStore(false)} />
                        </ModalWin.Header>
                        <ModalWin.Body>
                            {ShowStore &&
                                <CreditoArticulos
                ...
                {Estado.ShowFolio &&
                    <ModalWin open={Estado.ShowFolio} scrollable>
                        <ModalWin.Header>
                            ...
                            <button title='Cerrar' type="button" className="delete" onClick={() => DefinirEstado(s => ({ ...s, ShowFolio: false }))} />
                        </ModalWin.Header>
                        <ModalWin.Body>
                            {Estado.ShowFolio &&
                                <FolioConfirmacion
                ...
                <ModalWin open={ShowClientePerfil} center scrollable>
                    <ModalWin.Header>
                        <button type="button" className="delete" onClick={() => setShowClientePerfil(false)} />
                    </ModalWin.Header>
                    <ModalWin.Body>
                        {ShowClientePerfil &&
                            <Card Title="Detalles personales">
                                <Card.Body>
                                    <React.Fragment>
                                        <PerfilPersonaParaCapturaDeVales Editar={false} Persona={Estado.Datos.Persona as DBConfia_General.IPersonas_VW} Direcciones={Estado.Datos.Direcciones} Empleos={Estado.Datos.Empleos} ContCreditos={Estado.Datos.ContCreditos} Creditos={Estado.Datos.Creditos as DBConfia_Creditos.ICreditos_VW} oidc={props.oidc} ui={props.ui} />
                                    </React.Fragment>
                                </Card.Body>
                            </Card>
                        }
                ...
                {
                    verFirma && <ModalWin open={verFirma} center  >
                        <ModalWin.Header>
                            <h5 className={MODAL_TITLE_CLASS}>{`Firma ${DescripcionDistribuidor(1)}`}</h5>
                        ...
                        <ModalWin.Body>
                            ...
                            <label className='text-center p-3 border' id='drop-area'>
                                <input type="file" accept='image/*' id='input-file' hidden onChange={handleChange} />
                                <div id='img-view' ref={imageViewRef}>
                                    <img src="https://www.svgrepo.com/show/112825/cloud-upload-signal.svg" className='mt-3' alt="Subida de Imagen" width={'100px'} />
                                    <p className='fs-6'>Arrastre y suelte o apriete aqui <span className='fw-bold'>para subir una imagen</span></p>
                                </div>
                            </label>

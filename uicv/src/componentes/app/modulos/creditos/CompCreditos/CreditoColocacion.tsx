...
import { BuscarCreditoColocacion } from './CreditoColocacion/BuscarCreditoColocacion'
...
const CreditoColocacion = (props: CatalogosType) => {    // Controll our mounted state
const ExpPermisos = permisoExportar(2935); 
  ...
  const generarXLSX = () => {
    ...
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(state.Datos.map(reg=>({
      ZonaID: reg.ZonaID,
      Zona: reg.Zona,
      Sucursal_Nombre: reg.Sucursal_Nombre,
      SucursalID: reg.SucursalID,
      CoordinadorID: reg.CoordinadorID,
      CoordinadorNombre: reg.CoordinadorNombre,
      ValesColocados: reg.ValesColocados,
      Distribuidores: reg.Distribuidores,
      TotalColocado: reg.TotalColocado,
      SaldoActual: reg.SaldoActual,
      Capital: reg.Capital,
      interes: reg.interes,
      Seguro: reg.Seguro,
      TotalPrestado: reg.Total
  ...
  const Columns: IDataTableColumn[] =
    [
      ...
      },
      {
        ...
        minWidth: "20%",
        cell: (propss) => (
          <span className="text-center">{propss.CoordinadorNombre}</span>
        ),
      },
      {
        center: true,
        ...
        sortable: false,
        cell: (propss) => (
          <span className='text-center'>{propss.ValesColocados}</span>
        )
      },
      {
        center: true,
    ...
    <React.Fragment>
      <div className="row ">
        <div className="col-12">
          <Card Title="DETALLE DE COLOCACION">
            <Card.Body>
              <Card.Body.Content>
...
export default connect(mapStateToProps, mapDispatchToProps)(CreditoColocacion);
...
import * as FuncionesTicket from "./CreditoAplicaPagoClienteTickets/Funciones";
...
import { CForm } from "./CreditoAplicaPagoClienteTickets/CForm";
...
const CreditoAplicaPagoCliente = (props: CatalogosType) => {
  ...
  const [TicketIDs, setTicketIDs] = useState([] as number[]);
  const [ShowTickets, setShowTicket] = useState(false);
  const [DatosTicketMostrar, setDatosTicketMostrar] = useState([]);
  ...
  const GetTickets = (Datos: { ClienteID: number }) => {
    if (Datos.ClienteID > 0) {
      setGetTicket(true);
      setTicketIDs([]);
      FuncionesTicket.FNGetTickets(props.oidc, {
        ClienteID: Datos.ClienteID,
      })
        .then((respuesta: any) => {
          setShowTicket(true);
          setDatosTicketMostrar(respuesta);
          setGetTicket(false);
        ...
        .catch((error: any) => {
          toast.error(
            "Error al recuperar los Tickets, intente lo nuevamente o reportarlo a sistemas"
          );
          setGetTicket(false);
        });
  ...
  useEffect(() => {
    setDatosSeleccionados(
      filterArrayByValues(DatosTicketMostrar, TicketIDs, "MovimientoID")
    );
  }, [TicketIDs]);
                ...
                <ModalWin.Body>
                  {state.Mostrar && (
                    <CForm
                      ...
                      TicketsSeleccionados={DatosSeleccionados}
                    />
                  )}
                </ModalWin.Body>
              </ModalWin>
                ...
                <ModalWin open={ShowTickets} xlarge scrollable>
                  ...
                  <ModalWin.Body>
                    <DataTable
                      ...
                      selectableRowDisabled={(row: any) => (row.CatEstatusMovID == 1 || row.CatEstatusMovID == 2) ? false : true}
                      striped
                      dense
                      noHeader
                      responsive

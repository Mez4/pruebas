...
import * as FnUsuario from '../../seguridad/CompSeguridad/CompAdministracionUsuarios/Funciones'
import ModalWin, { MODAL_TITLE_CLASS } from '../../../../global/ModalWin'

// Icons
import { FaPencilAlt, FaPlus, FaSearch, FaKey, FaUserCheck, FaUserTimes } from 'react-icons/fa'

// Custom components
import { Card, Spinner } from '../../../../global'
import { CForm } from './CreditoGrupoUsuarios/CForm'
...
const CreditoGrupoUsuarios = (props: CatalogosType) => {
    const MySwal = withReactContent(Swal)
    let isMounted = React.useRef(true)
    const DatosDefecto = { UsuarioID: 0 }
    const DatosDetalleDefecto = { GrupoID: 0, UsuarioID: 0, Estatus: true }
    const Datos: any[] = []
    const DatosMostrar: any[] = []
    const DatosDetalle: DBConfia_Creditos.IGruposUsuarios_VW[] = []
    ...
    const FnCambiarEstt = (GrupoID: number, UsuarioID: number, Estatus: boolean) => {
        Funciones.FNUpdate(props.oidc, { GrupoID, UsuarioID, Estatus })
            .then((respuesta: any) => {
                toast.success('Se cambio el acceso al grupo')
            ...
            { name: 'Estatus', selector: 'Bloqueado', sortable: true, cell: (props) => <span>{props.Bloqueado ? "Inactivo" : "Activo"}</span> },
            {
                ...
                cell: (data) =>
                    <div style={{ overflowX: 'auto', whiteSpace: 'nowrap' }}>
                        <button title='Permisos' data-tip data-for={`PermisosTooltip${data.UsuarioID}`} className="asstext" style={{ margin: '.15em', width: '15%', height: '40px', padding: '0px', tableLayout: 'fixed', borderCollapse: 'collapse' }} type="button" onClick={() => {
    ...
    const DetailColumns: IDataTableColumn[] =
        [
            { name: 'Id', width: '95px', selector: 'GrupoID', sortable: true },
            { name: 'Producto', selector: 'Producto', sortable: true },
            { name: 'Sucursal', selector: 'Sucursal', sortable: true },
            { name: 'Coordinador', selector: 'NombreCompleto', sortable: true },
            { name: 'Clasificador Grupo', selector: 'Descripcion', sortable: true },
            { name: 'Estatus', selector: 'EsttGrupo', sortable: true, cell: (props) => <span>{props.EsttGrupo ? "Activo" : "Inactivo"}</span> },
            { name: 'Acceso', selector: 'Estatus', sortable: true, cell: (props) => <span>{props.Estatus ? "Si" : "No"}</span> },
            {
                name: 'Acciones', sortable: false,
                cell: (data) =>
                    <div style={{ overflowX: 'auto', whiteSpace: 'nowrap' }}>
                        <button data-tip data-for={`EstTooltip${data.GrupoID}`} className="asstext" style={{ margin: '.15em', width: '15%', height: '40px', padding: '0px', tableLayout: 'fixed', borderCollapse: 'collapse' }} type="button" onClick={() => {
                            MySwal.fire({
                                title: '<strong>Quitar Grupo</strong>',
                                icon: 'question',
                                html:
                                    <div className="text-center">
                                        {data.Estatus ? 'Se desactivara el permiso al grupo ¿Desea continuar?' : 'Se activara el permiso al grupo ¿Desea continuar?'}
                                    </div>,
                                ...
                                if (result.isConfirmed) {
                                    FnCambiarEstt(data.GrupoID, data.UsuarioID, data.Estatus ? false : true)
                                }
                            })
                        }}>
                            {!data.Estatus && <FaUserCheck />}
                            {data.Estatus && <FaUserTimes />}
                        </button>
                        <ReactTooltip id={`EstTooltip${data.GrupoID}`}
                            type="info"
                            effect="solid"
                            clickable
                            globalEventOff="click"
                        >
                            {data.Estatus ? 'Desactivar' : 'Activar'}
                        </ReactTooltip>
                    </div>
            },
        ]
    ...
    const cbActualizarDetalle = (item: any) => {
        setState({ ...state, DatosDetalle: state.DatosDetalle.map(Dato => Dato.GrupoID === item.GrupoID && Dato.UsuarioID === item.UsuarioID ? item : Dato) })
    }

    return (
        <div className="row">
            <div className="col-12">
                <Card Title="Administrar Permisos Grupos">
                    <Card.Body>
                        <Card.Body.Content>
                            ...
                            {!state.Cargando && !state.Error &&
                                <div>
                                    ...
                                    <ModalWin open={state.Detalle} scrollable large>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>
                                                {"Permisos Grupos"}
                                            </h5>
...
export default connect(mapStateToProps, mapDispatchToProps)(CreditoGrupoUsuarios)
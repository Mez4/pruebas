...
import * as Funciones from './CreditoCondicionSucursal/Funciones'
import * as FnProductos from './CreditoProducto/Funciones'
import * as FnCondiciones from './CreditoCondicion/Funciones'
...
// Custom components
import { ActionSelect, Card, CustomActionSelect, Spinner } from '../../../../global'
import { CForm } from './CreditoCondicionSucursal/CForm'
import { FiRefreshCcw } from 'react-icons/fi'
import { FiltrarDatos } from '../../../../../global/functions'
import { Formik, Form } from 'formik'
import * as Yup from 'yup';
import { DBConfia_Creditos } from '../../../../../interfaces_db/DBConfia/Creditos'
import { FNGetByProducto, FNGetCondicionesFiltro } from './CreditoCondicion/Funciones';
...
const CreditoCondicionSucursal = (props: CatalogosType) => {
    ...
    const optCondicionesFiltro: any[] = []
    ...
    const FnGetCondiciones = (ProductoID?: number) => {
        FnCondiciones.FNGetByProducto(props.oidc, ProductoID)
            .then((respuesta: any) => {
                // if (isMounted.current === true) {

                var condiciones = respuesta.map((valor: any) => {
                    var obj = { value: valor.CondicionesID, label: valor.Descripcion };
                    return obj
                });
    ...
    const FnGetCondicionesFiltro = (ProductoID?:number) => {
        FnCondiciones.FNGetCondicionesFiltro(props.oidc, ProductoID)
            .then((respuesta: any) => {
                // if (isMounted.current === true) {
                ...
                setState(s => ({ ...s, optCondicionesFiltro: condiciones }))
                // }
            })
            .catch(() => {
                // if (isMounted.current === true) {
                setState(s => ({ ...s, optCondicionesFiltro: [] }))
                // }
            })
    }
    ...
    const Columns = React.useMemo(() => {
        let colRet: IDataTableColumn[] =
            [
                ...
                { name: 'Condicion', selector: 'Descripcion', sortable: true, wrap: true, },
    ...
    const refCondicion = useRef<Select>(null)
    ...
    const clearFormByLevel = (level: number) => {
        if (level === 0) {
            ...
            refCondicion.current?.select.setValue({ value: '0', label: '' }, "deselect-option")
    ...
    return (
        <div className="row">
            <div className="col-12">
                <Card Title="Administrar Relaciones Sucursales - Condiciones">
                    <Card.Body>
                        <Card.Body.Content>
                            <Formik
                            initialValues={{
                            ...
                            onSubmit={(values: any) => {
                                setLoading(true)
                                setState(s => ({ ...s, Cargando: true}))
                                FNGetLocal(values.ProductoID, values.SucursalID, values.CondicionesID)
                            }}>
                            {({ values }) => (
                                <Form>
                                            ...
                                            <div className="row" style={{ textAlign: 'initial' }}>
                                                ...
                                                <ActionSelect
                                                    ...
                                                    <ActionSelect
                                                        ...
                                                        options={state.optCondicionesFiltro}
                                                        addDefault={true}
                                    ...
                                    <ModalWin open={state.Form.Mostrar} large={true}>
                                        ...
                                        <ModalWin.Body>
                                            <CForm
                                                ...
                                                // CondicionesID={state.Form.CondicionesID}
...
export default connect(mapStateToProps, mapDispatchToProps)(CreditoCondicionSucursal)
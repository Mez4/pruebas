...
export const CForm = (props: CFormType) => {
  ...
  const [getTicket, setGetTickets] = useState(false);
  ...
  const Columns = React.useMemo(() => {
    let colRet: IDataTableColumn[] = [
      ...
      { name: "GestorCaptura", selector: "NombreGestorRegistra", sortable: true, width: '20%', center: true },
  ...
  const GetTickets = (Datos: { DistribuidorId: number }) => {
    // console.log('Datos: ', Datos)
    if (Datos.DistribuidorId > 0) {
      setGetTickets(true);
      setMovimientoIDs([]);
      Funciones.FNGetTickets(props.oidc, {
  ...
  return (
    <>
      <Formik
        ...
        onSubmit={(values: any) => {
          setLoading(true);
          Funciones.FNAddPagoTicket(props.oidc, {
            ...values,
            ProductoID: props.ProductoID,
            MovimientoIDs,
      ...
      {ShowTicket && (
        <ModalWin open={ShowTicket} xlarge scrollable>
          <ModalWin.Header>

...
import CreditoCondicionDetalle from './CreditoCondicionDetalle'
import * as Funciones from './CreditoCondicion/Funciones'
...
// Icons
import { FaPencilAlt, FaPlus, FaSearch, FaListAlt, FaWindowClose, FaPercentage, FaChartBar } from 'react-icons/fa'

// Custom components
import { Card, Spinner } from '../../../../global'
import { CForm } from './CreditoCondicion/CForm'
import { FiRefreshCcw } from 'react-icons/fi'
import { FiltrarDatos } from '../../../../../global/functions'

import ReactTooltip from 'react-tooltip';
import { CFormTraspasoTabulador } from './CreditoCondicion/CFormTraspasoTabulador'
import { CFormTraspasoTabuladorNivOrig } from './CreditoCondicion/CFormTraspasoTabuladorNivOrig'

type CatalogosType = {
    oidc: IOidc
}

const CreditoCondicion = (props: CatalogosType) => {
    ...
    const Columns = React.useMemo(() => {
        let colRet: IDataTableColumn[] =
            [
                { name: 'Id', selector: 'CondicionesID', sortable: true, wrap: true,width: '50px', center: true, },
                ...
                { name: 'Activa', selector: 'Activo', sortable: true, cell: (props) => <span>{props.Activo ? "SI" : "No"}</span>, width:'60px', center: true, },
                { name: 'Registró', selector: 'UsuarioRegistro.Nombre', sortable: true,wrap: true,center: true,  },
                { name: 'Fecha Registro', selector: 'RegistroFecha', sortable: true, wrap: true, width:'110px',center: true,  },
                { name: 'Modificó', selector: 'UsuarioModifico.Nombre', sortable: true,wrap: true, center: true, },
                { name: 'Fecha Modificación', selector: 'ModificoFecha', sortable: true,wrap: true,width:'110px', center: true, },
                {
                    name: 'Acciones', sortable: false, width:'75px',
                    center: true,
                    cell: (props) =>
                        <div>
                            ...
                            {'\u00A0'}
                            <button title='Detalle' data-tip data-for="DetailTooltip" className="asstext" type={"button"} onClick={() => {
                                setState(s => ({
                                    ...s,
                                    ShowDetail: true,
                                    ProductoID: props.ProductoID,
    ...
    const cbActualizar = (item: any) =>
        setState(s => ({
            ...s, Datos: state.Datos.map(Dato => Dato.CondicionesID === item.CondicionesID ? item : Dato), Form: {
                ...state.Form, Mostrar: false,
                Datos: DatosDefecto
            }, isUpdate: false
        }))
    ...
    return (    
        <div className="row">
            <div className="col-12">
                <Card Title="Administrar Condiciones">
                    <Card.Body>
                        <Card.Body.Content>
                            ...
                            {!state.Cargando && !state.Error &&
                                <div>
                                    <DataTable
                                            ...
                                            <div className="row">
                                                <div className="col-sm-12">
                                                    <div className="input-group mb-3">
                                                        <input type="text" className="form-control" placeholder="Buscar condición" value={state.Filtro} onChange={e => setState(s => ({ ...s, Filtro: e.target.value }))} />
                                        ...
                                        responsive
                                        keyField={"Index"}
                                        defaultSortField={"CondicionesID"}
                                        columns={Columns}
                                    />
                                    <ModalWin open={state.Form.Mostrar} large={true}>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>
                                                {(state.Form.Id) ? "Editar Condición" : "Agregar Condición"}
                                            </h5>
                                        </ModalWin.Header>
                                        <ModalWin.Body>
                                            <CForm
                                    ...
                                    <ModalWin open={state.ShowDetail} full={true}>
                                        <ModalWin.Header>
                                            ...
                                            <button title='Cerrar' type="button" className="delete mr-5" onClick={() => setState(s => ({ ...s, ShowDetail: false }))} />
                                        </ModalWin.Header>
                                        <ModalWin.Body>
                                            {state.ShowDetail &&
                                                <CreditoCondicionDetalle
                                    ...
                                    <ModalWin open={state.ShowTab} center>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>
                                                Traspaso Condiciones
                                            </h5>
                                        ...
                                        <ModalWin.Body>
                                            {state.ShowTab &&
                                                ...
                                                ProductoID={state.ProductoID}
                                                CondicionesID={state.CondicionesID}
                                                //optCondiciones={state.optCondiciones}
                                    ...
                                    <ModalWin open={state.ShowTabOrigen} large={true} center={true}>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>
                                                Traspaso Condiciones Por Origen
                                            </h5>
                                            <button title='Cerrar' type="button" className="delete" onClick={() => {
                                        ...
                                        <ModalWin.Body>
                                            {state.ShowTabOrigen &&
                                                ...
                                                ProductoID={state.ProductoID}
                                                CondicionesID={state.CondicionesID}
                                                CondicionesDestinoID={state.CondicionesID}
...
export default connect(mapStateToProps, mapDispatchToProps)(CreditoCondicion)
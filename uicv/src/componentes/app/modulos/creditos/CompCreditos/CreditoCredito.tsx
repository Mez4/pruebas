...
import { BuscarCreditos } from "./CreditoCredito/BuscarCreditos";
...
const CreditoCredito = (props: CatalogosType) => {
  ...
  const FNAutorizar = (CreditoID: number) => {
    Funciones.FNAutorizar(props.oidc, CreditoID)
      .then((respuesta: any) => {
        if (respuesta.res == 1) {
          toast.success(respuesta.msj);
  ...
  const FNDesembolsar = (CreditoID: number, TipoDesembolsoID: number) => {
    setState((s) => ({
      ...s,
      CajaDefault: {
        ProductoID: 0,
    ...
    Funciones.FNModificarSucCaja(props.oidc, Datos)
      .then((respuesta: any) => {
        if (respuesta.res == 1) {
          console.log('caja', state.CajaDefault.CajaID)
          Funciones.FNDesembolsar(props.oidc, CreditoID, Datos.CajaID)
            .then((respuesta: any) => {
              if (respuesta.res == 1) {
                toast.success(respuesta.msj);

                FnBancos.FNGet(props.oidc, TipoDesembolsoID)
                  .then((res: any) => {
                    if (res.FormatoImpresionExtra) {
                      toast.info(
            ...
            .catch(() => {
              toast.error(
                "Error al desembolsar el crédito, vuelva a intentarlo."
              );
            });
        } else {
          toast.warning(respuesta.msj);
  ...
  const HiddenColumns: IDataTableColumn[] = [
    {
      ...
      cell: (data) => (
        ...
        >
          ...
          <button
            data-tip="true"
            data-for={`EditarTooltip${data.CreditoID}`}
            disabled={
              data.EstatusID != "P" || state.tipoUsuario == 6 ? true : false
            }
            className="asstext"
            type={"button"}
            style={{
            ...
            onClick={() => {
              MySwal.fire({
                title: "<strong>Modificar Crédito</strong>",
                icon: "question",
                html: (
          ...
          <ReactTooltip
            id={`EditarTooltip${data.CreditoID}`}
            type="info"
            effect="solid"
            clickable
            globalEventOff="click"
          ...
          <button
            disabled={data.EstatusID != "P" ? true : false}
            data-tip="true"
            data-for={`DesembolsoTooltip${data.CreditoID}`}
            className="asstext"
            style={{
              margin: ".15em",
              width: "10%",
            ...
            onClick={() => {
              MySwal.fire({
                ...
                html: (
                  <div className="text-center">
                    Se desembolsara el crédito ¿Desea continuar?
                  </div>
                ),
                showCloseButton: false,
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                  ...
                  // FNDesembolsar(data.CreditoID, data.TipoDesembolsoID)
                }
              });
            }}
          >
          ...
          <ReactTooltip
            id={`DesembolsoTooltip${data.CreditoID}`}
            type="info"
            effect="solid"
            clickable
            globalEventOff="click"
          ...
          <button
            title=""
            data-tip
            data-for={`AgregarProductoTienditaTooltip${data.CreditoID}`}
            disabled={data.SucursalID != data.SucursalOrigenID}
            className="asstext"
            style={{
              margin: ".15em",
              width: "10%",
          ...
          <ReactTooltip
            id={`AgregarProductoTienditaTooltip${data.CreditoID}`}
            type="info"
            effect="solid"
            clickable
            globalEventOff="click"
          >
            Agregar Productos Tiendita
          </ReactTooltip>
        </div>
      ),
    },
  ...
  React.useEffect(() => {
    if (!state.ShowCaja && state.Desembolso.Desembolsar)
      FNDesembolsar(
        ...
        state.Desembolso.TipoDesembolsoID
      );
  }, [state.ShowCaja]);
  ...
  const fnSetearArticuloValidado = () => {
    ...
    setShowFolio(false);
    fnAgregarArticuloTienditaACredito(true);
  };

  const fnVerificarArticuloTienditaRequiereValidacionSms = () => {
    if (articles.length) {
      var EsArticuloEspecial = 0;
      articles.map((Dato: any) => {
      ...
      if (EsArticuloEspecial === 0) {
        fnAgregarArticuloTienditaACredito(false);
      } else {
        setShowFolio(true);
      }
    }
  ...
  const fnAgregarArticuloTienditaACredito = (validado: boolean) => {
    FnVales.FNCheckBox(props.oidc, state.CajaDefault.CajaID).then(
      (res: any) => {
        if (!res.Cerrada) {
          FnVales.FNAgregarArticuloCredito(props.oidc, {
            CreditoID: creditoSeleccionado!.CreditoID,
            DistribuidorId: creditoSeleccionado!.DistribuidorID,
            ClienteId: creditoSeleccionado!.ClienteID,
            CajaID: state.CajaDefault.CajaID,
            SucursalId: creditoSeleccionado!.SucursalID,
            TipoCanje: 1,
            ...
            .then((respuesta: any) => {
              if (respuesta.status || respuesta.regresa === 1) {
                toast.success(
                  `Se modificó el crédito con el N° ${creditoSeleccionado!.CreditoID
                  }`
                );

                cbActualizar(respuesta.Credito);
                FNGetDetalleTiendita(
                  creditoSeleccionado!.CreditoID,
                  creditoSeleccionado
                );
              } else {
                toast.warning(respuesta.msj);
  ...
  useEffect(() => {
    fnVerificarArticuloTienditaRequiereValidacionSms();
  }, [shopInfo]);
    ...
    >
      <div className="row">
        <div className="col-12">
          <Card Title="Administrar Créditos">
            <Card.Body>
              <Card.Body.Content>
                ...
                {ShowStore && !!creditoSeleccionado && (
                  <ModalWin open={ShowStore} large center scrollable>
                    <ModalWin.Header>
                      <h5 className={MODAL_TITLE_CLASS}>
                        Agregar Artículos a Crédito -{" "}
                        {creditoSeleccionado.CreditoID}
                      </h5>
                      <button
                        type="button"
                    ...
                    <ModalWin.Body style={{ overflowY: "unset" }}>
                      <div className="columns is-desktop is-tablet mb-1">
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">Socia:</strong>
                          {creditoSeleccionado.DistribuidorID} -
                          {creditoSeleccionado.Distribuidor}
                        </p>
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">Cliente:</strong>
                          {creditoSeleccionado.ClienteID} -
                          {`${creditoSeleccionado.Nombre} ${creditoSeleccionado.ApellidoPaterno} ${creditoSeleccionado.ApellidoMaterno}`}
                        </p>
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">Sucursal:</strong>
                          {creditoSeleccionado.SucursalNombre}
                        </p>
                      </div>
                      <div className="columns is-desktop is-tablet mb-1">
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">N° Crédto:</strong>
                          {creditoSeleccionado.CreditoID}
                        </p>
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">Total actual:</strong>
                          {formatter.format(creditoSeleccionado.ImporteTotal)}
                        </p>
                        <p className="column is-12-mobile is-12-tablet is-4-desktop">
                          <strong className="mr-1">Folio:</strong>
                          {creditoSeleccionado.ValeCanje}
                        </p>
                      </div>
                      <CreditoArticulos
                        SucursalId={state.CajaDefault.SucursalID}

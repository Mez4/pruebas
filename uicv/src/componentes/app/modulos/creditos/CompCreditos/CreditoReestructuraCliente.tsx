...
import * as Funciones from './CreditoReestructuraCliente/Funciones'
// import ModalWin, { MODAL_TITLE_CLASS } from '../../../../global/ModalWin'

// Icons
import { FaCheck, FaExclamationCircle, FaFile, FaPencilAlt, FaPlus, FaSearch } from 'react-icons/fa'

// Custom components
import { Card, ModalWin, Spinner } from '../../../../global'
import { CForm } from './CreditoReestructuraCliente/CForm'
...
import { CFormR } from './CreditoReestructuraCliente/CFormR'
...
const ReestructuraCliente = (props: CatalogosType) => {
        ...
        let colRet: IDataTableColumn[] =
            [
                ...
                },
                {
                    ...
                    selector: 'Capital',
                    cell: (props) => <span>{formatter.format(props.Capital)}</span>
                },
                {
    ...
    const ColumnsM = React.useMemo(() => {
        let colRet: IDataTableColumn[] =
            [
                ...
                {
                    name: 'FechaHoraUltimoPago',
                    selector: 'FechaHoraUltimoPago',
                    cell: (props) => <span>{moment(props.FechaHoraUltimoPago).format('DD/MM/YYYY')}</span>
                },
                {
                    name: 'CapitalPagado',
                    selector: 'CapitalPagado',
                    cell: (props) => <span>{formatter.format(props.CapitalPagado)}</span>
                },
                {
                    name: 'SaldoActual',
                    selector: 'SaldoActual',
                    cell: (props) => <span>{formatter.format(props.SaldoActual)}</span>
                },
            ]
        return colRet
    }, [state.Form])

    const cbActualizaDatoCliente = (item: any) => {
        setState(s => ({ ...s, DatosMostrarCliente: [] }))
        setState(s => ({ ...s, DatosMostrarCliente: item }))
    }

    const selectClienteMultiple = (clientesIds: any) => {
        let clienteIdsAux = clientesIds
        setState(s => ({
            ...s,
            ClientesX: clienteIdsAux,
        }))
    }
    ...
    const cbImporteDatos = (importe: number) => {
    ...
    const cbLimpiaTablas = (item: any, importe: 0, plazos: 0) => {
        setState(s => ({ ...s, DatosMostrarCliente: [], Datos: [], Mostrar: false, Form: { ...s.Form, Importe: importe, Plazos: plazos }, ClientesX: [] }));
    }
    ...
    return (
        <>
            <Card Title="Solicitud Reestructura Cliente">
                <Card.Body>
                    <Card.Body.Content>
                        <div>
                            <CForm
                                ...
                                DatosMostrarCliente={state.DatosMostrarCliente}
                                initialValues={state.Form}
                                optPlazos={state.optPlazos}
                                cbActualizaDatos={cbActualizaDatos}
                                cbMuestrCotizacion={cbMuestrCotizacion}
                                cbActualizaDatoCliente={cbActualizaDatoCliente}
                                selectClienteMultiple={selectClienteMultiple}
                                cbMuestraActual={cbMuestraActual}
                                cbImporteDatos={cbImporteDatos}
                            />
                        </div>
                    </Card.Body.Content>
                </Card.Body>
            ...
            {state.Datos.length > 0 && <div className="row">
                <div className="col-12">
                        ...
                        <Card.Body>
                            <Card.Body.Content>
                                ...
                                <div className="text-end" style={{ margin: '2em' }}>
                                    ...
                                    >REESTRUCTURAR SALDO DE CLIENTE(S)</button>
                                </div>
                            </Card.Body.Content>
                        </Card.Body>
                    </Card>
                    <ModalWin open={state.Mostrar} large>
                        <ModalWin.Header>
                            <h5 className={MODAL_TITLE_CLASS}>SOLICITAR REESTRUCTURA CLIENTE</h5>
                        ...
                        <ModalWin.Body>
                            {<CFormR
                                ...
                                initialValues={state.Form}
                                ClientesX={state.ClientesX}
                                cbMuestrCotizacion={cbMuestrCotizacion}
                                cbLimpiaTablas={cbLimpiaTablas}
                                cbMuestraActual={cbMuestraActual}
                            />}
                        </ModalWin.Body>
                    </ModalWin>
                        ...
                        <table className='table table-bordered'>
                            <thead>
                                <tr>
                                    ...
                                    <th>{state.solActual?.MachoteFilePath ? <FaCheck color='green' /> : <FaExclamationCircle color='red' />} Machote Clientes</th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" style={{ paddingTop: '0px', paddingBottom: '3px' }} className="btn btn-link" onClick={() => fnVerDoc(state.solActual?.MachoteFilePath ?? '', 'MACHOTE CLIENTES')}>
                                            VER DOCUMENTO
                                        </button>
                                    </td>
                                    <td>
...
export default connect(mapStateToProps, mapDispatchToProps)(ReestructuraCliente)
...
export default function FormDistPagoVencimiento(
  ...
  const [rowsPerPage, setRowsPerPage] = useState(500);  // Filas por página
  ...
  itemsPerPage: 500, // Registros por página
  ...
  const filtrar = (values: any) => {
    ...
    if (diaVerificar !== "05" && diaVerificar !== "20") {
      ...
      Funciones.FNReporte1625_3(props.oidc, {
            ...
            respuesta.map((element: any) => {
              ...
              if (diaCorte == "05") {
                Reporte = {
                  ...
                  FechaConvenio:
                    element.FechaConvenio == null
                      ? null
                      : moment(element.FechaConvenio).format("DD/MM/YYYY"),
  ...
  return (
    <div>
      ...
      {state.Datos.length ?
        (<DataGridComp
        data={state.Datos}/> ) : <span style={{textAlign:'center'}}>NO HAY DATOS POR MOSTRAR</span> }
    </div>
  );}

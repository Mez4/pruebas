...
type CFormType = {
    ...
    DatosMostrarCliente: any[]
    columns: IDataTableColumn<any>[]
    cbActualizaDatos(item: any): any
    cbImporteDatos(item: any): any
    selectClienteMultiple(clientesIds: any): void
    cbActualizaDatoCliente(item: any): any
...
export const CForm = (props: CFormType) => {
    ...
    const [ClientesX, setClientesX] = React.useState([] as number[])

    const [valuePlazos, setValuePlazos] = React.useState(0)
    ...
    const getClientes = async (DistribuidorId: number) => {
        if (DistribuidorId > 0) {
            Funciones.FNGetClientes(props.oidc, {
                ...
                .then((respuesta: any) => {
                    ...
                    setLoading(false)
                    props.cbActualizaDatoCliente(respuesta)
                ...
                .catch((error: any) => {
                    ...
                    toast.error(`Error al recuperar los clientes, intente lo nuevamente o reportarlo a sistemas. ${error}`)
    ...
    const selectByCheck = (row: any) => {
        let clientesSaldo = row.selectedRows.map((valor: any) => {
            return valor.SaldoActual;
        })

        console.log('SALDO', clientesSaldo)
        const sum = clientesSaldo.reduce((partialSum, a) => partialSum + a, 0);
        console.log('SUMS', sum);
        props.cbImporteDatos(sum)

        let clientessSelected = row.selectedRows.map((valor: any) => {
            return valor.CreditoID;
        })

        let clientesMostrar = props.DatosMostrarCliente.map((valor: any) => {
            return valor.CreditoID;
        })

        ClientesX.forEach(e => {
            if (!clientesMostrar.includes(e))
                clientessSelected.push(e)
        })

        setClientesX(clientessSelected);
        props.selectClienteMultiple(clientessSelected);
        props.cbMuestrCotizacion([], 0, valuePlazos);
    }
        ...
        <>
            <Formik
                ...
                onSubmit={(values: any) => {
                    ...
                    console.log('##', values)
                    console.log('##', ClientesX)
                    //setValuePlazos(values.Plazos)
                    if (ClientesX.length == 0) {
                        toast.error('Selecciona por lo menos un Cliente de la Socia Dv')
                        return
                    }
                    setLoading(true)
                ...
                {({ values }) => (
                    <Form>
                        <div className="columns is-desktop is-tablet">
                            ...
                            <div className="column is-12-mobile is-12-tablet is-3-desktop">
                                <Distribuidores disabled={values.SucursalId <= 0} SucursalID={values.SucursalId} name={'DistribuidorId'} WithProducto RequiereSuc cbAccion={
                                    (val) => {
                                        getClientes(val);
                            ...
                            <div className="column is-12-mobile is-12-tablet is-3-desktop">
                                <ActionSelect
                                    ...
                                    addDefault={true}
                                    valor={0}
                                    accion={(val: number) => setValuePlazos(val)}
                                />
                            </div>
                        ...
                        <div className="columns is-desktop is-tablet">
                            <div className="column is-12-mobile is-12-tablet is-12-desktop">
                                {ClientesX.length === 0 &&
                                    <div className="text-danger">*Seleccione por lo menos un Cliente de la lista</div>
                                }
                            </div>
                        </div>
                        <div className="columns is-desktop is-tablet">
                            <div className="column is-12-mobile is-12-tablet is-12-desktop">
                                <DataTable
                                    ...
                                    data={props.DatosMostrarCliente}

...
const CatalogoEstatus = (props: CatalogosType) => {    // Controll our mounted state
    ...
    const cbActualizar = (item: any) => {
        ...
        setState({ ...state, Datos: state.Datos.map(Dato => Dato.EstatusID === item.EstatusID ? item : Dato), Form: { ...state.Form, Mostrar: false, Datos: DatosDefecto } })
    }
    ...
    const Columns:/*: React.useMemo(() => {
        ...
        [
            { name: 'Id', selector: 'EstatusID', sortable: true, width: '20%' },
            { name: 'Clave', selector: 'Clave', sortable: true, width: '20%' },
            { name: 'Descripción', selector: 'Descripcion', sortable: true,width: '40%' },
            {
                name: 'Acciones',
                sortable: false,
                width: '20%',
                cell: (fila) =>
                    <button className="asstext" type={"button"} onClick={() => {
                        setState(s => ({
                            ...
                            Form: {
                                Mostrar: true,
                                Datos: { ...DatosDefecto, EstatusID: fila.EstatusID, Clave: fila.Clave, Descripcion: fila.Descripcion },
                                Id: fila.EstatusID
                            }
                        }))
                    }}>
                            ...
                            {!state.Cargando && !state.Error &&
                                <div>
                                                ...
                                                <div className="col-sm-12">
                                                    <div className="input-group mb-3">
                                                        ...
                                                        <button className="btn btn-outline-secondary" type="button"
                                                            onClick={() => setState({ ...state, Form: { Mostrar: true, Datos: { EstatusID: 0, Clave: '', Descripcion: '' }, Id: undefined } })}
                                                        ><FaPlus /></button>
                                    ...
                                    <ModalWin open={state.Form.Mostrar} center={true}>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}> {state.Form.Id != null ? "EDITAR ESTATUS" : "AGREGAR ESTATUS"} </h5>
                                        </ModalWin.Header>
                                        <ModalWin.Body>
                                            <CForm
                                                oidc={props.oidc}

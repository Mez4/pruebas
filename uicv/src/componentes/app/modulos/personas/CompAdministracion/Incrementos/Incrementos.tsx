...
const GenerarColumna = (Columna: EColumnas): IDataTableColumn => {
    switch (Columna) {
        ...
        case EColumnas.IncrementoQuincena:
            return ({ name: 'Incremento', selector: 'IncrementoQuincena', sortable: true, cell: (cprops) => <span>{FormateoDinero.format(cprops.IncrementoQuincena)}</span> })
    }
}
...
const Incrementos = (props: IncrementosTipo) => {

    const [Estado, definirEstado] = useState<IncrementosEstadoTipo>({
        Contratos: [],
        Cargando: true,
        Error: false,
        // Type: 1,
        ...
        try {
            ...
            var Contratos = res.map(c => {
                return {
                    ...
                    IncrementoQuincena: c.IncrementoQuincena,
                    Type: 1,
                }
            })
    ...
    const Columns = //React.useMemo(
        () => {
            ...
            if (props.Funciones.length > 0)
                Columnas.push({/*
                    name: 'Incrementar', selector: 'LineaCredito', sortable: false, minWidth: `${props.Funciones.length * 50}px`,
                    cell: (cprops) =>
                        <div>
                            {props.Funciones.map((Fn, FnId) =>
                                <Formik
                                    ...
                                    validationSchema={Yup.object().shape({
                                        ...
                                        IncrementoQuincena: Yup.number().required('Ingrese un monto mayor a 0').moreThan(0, 'El monto debe ser mayor a 0').max(cprops.IncrementoQuincena, `El monto maximo debe ser menor o igual a ${FormateoDinero.format(cprops.IncrementoQuincena)}`),
                                    ...
                                    onSubmit={async (values: any, { resetForm }) => {
                                        ...
                                        try {
                                            ...
                                            var res: any = await FnIncrementos.FNIncrease(props.oidc, { ...values, msj: '', regresa: 0 })
                                            ...
                                            if (res.regresa == 1) {
                                                ...
                                                toast.success(`Se incrementó la linea del crédito a ${res.LineaCredito}`)
                                        ...
                                        {/*<div className="btn-group">
                                            <span style={(loading || !cprops.Activo) ? { pointerEvents: "none", opacity: "0.4" } : {}} key={`fn_${cprops.DistribuidorID}_${FnId}`} >
                                                {cprops.Type == 1 && <>
                                                    ...
                                                    }}><IoAddSharp color={cprops.Activo ? 'green' : 'gray'} size={20} /></button>
...
Incrementos.EColumnas = EColumnas

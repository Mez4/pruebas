...
//import VerActaFallecimiento from './VerActaFallecimiento'
import VerActa from './VerActa'
...
type CatalogosSolicitudSociaType = {
    ...
    SolicitudesFallecidasSeleccionadas: [],
    CantidadSolicitudesFallecidasSeleccionadas: number,
    DistribuidorID: number,
}

const SolicitudesFallecimiento = (props: CatalogosSolicitudSociaType) => {
    ...
    const fnVerEvidencia = (id , ruta) => setState({...state, Form: { ...state.Form }, MostrarEvidencias : true, DistribuidorID : id, Ruta : ruta });
    const fnCancelarEvidencia = () => setState({...state, Form : {...state.Form }, MostrarEvidencias : false});


    const FNGetLocal = () => {
        setState(s => ({ ...s, Cargando: true }))
        Funciones.FNGetV2(props.oidc).then((respuesta: any) => {
    ...
    const fnGetFiltroEstatus = (EstatusID: number) => {
        ...
        Funciones.FNGet(props.oidc).then((res: any) => {
            console.log(res);
            if (isMounted.current === true) {
                console.log("###", res);
    ...
    const contextActions = React.useMemo(() => {
        ...
        const handleClick = () => {
            ...
            MySwal.fire({
                title: '<strong>Aceptación de Solicitudes de Fallecimiento</strong>',
                icon: 'info',
                html:
                ...
                if (result.isConfirmed) {
                    MySwal.fire({
                        title: '<strong>Solicitudes de Fallecimiento</strong>',
                        icon: 'warning',
                        html:
                            <div className="text-center">
                                <br />
        ...
        //         <BsFillBarChartFill />  Aceptar Solicitudes Seleccionados
        //     </button>
        // );
    }, [selectedRows]);

    const disableRow = (item: any) => {
        if (item.EstatusID == 3 || item.EstatusID == 2) {
            return true
        } else {
            return false
        }
    ...
    const AceptarSolicitud = (item: any) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                Funciones.FNAceptarFallecida(props.oidc, item)
                    .then((resp: any) => {
                            ...
                            MySwal.fire(
                                {
                                    ...
                                    html: <div><br />
                                        <h3 className="text-center">Aviso</h3>
                                        <div className={`modal-body`}>
                                            <h5 className="text-center">Se acepto la solicitud de Fallecimiento</h5>
                                        </div>
                                    </div>,
                        ...
                        MySwal.fire(
                            {
                                ...
                                html: <div><br />
                                    ...
                                    <div className={`modal-body`}>
                                        <h5 className="text-center">Ocurrió un problema al aceptar la solicitud.</h5>
                                    </div>
                                </div>,
                                confirmButtonText: `Aceptar`,
                                allowOutsideClick: false,
    ...
    const CancelarSolicitud = (item: any) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                Funciones.FNCalcelarSolicitudFallecida(props.oidc, item)
                    .then((resp: any) => {
                        console.log("resp: ", resp)
                        setLoading(false)
                        if (resp.EstatusID == 1) {
                            MySwal.fire(
                                {
                                    ...
                                    html: <div><br />
                                        <h3 className="text-center">Aviso</h3>
                                        <div className={`modal-body`}>
                                            <h5 className="text-center">Se canceló la Solicitud de Fallecimiento</h5>
                                        </div>
                                    </div>,
                                    //timerProgressBar: true,
                                    confirmButtonText: `Aceptar`,
                ...
                cell: (propss) =>
                    <div className='divInDTable text-center radiusSmallDivR' data-tip data-for={`VE_${propss.SolicitudID}`}>
                        <button
                            ...
                            onClick={() => { fnVerEvidencia(propss.DistribuidorID, propss.Ruta)  }}>
                            <FaEye />
                            <ReactTooltip id={`VE_${propss.DistribuidorID}`} type="info" effect="solid" >Ver Evidencia</ReactTooltip>
                        </button>
                    </div>
    ...
    return (
        <>
                                ...
                                {!state.Cargando && !state.Error &&
                                    <div>
                                        ...
                                        <DataTable
                                            ...
                                            title={<span>Lista de Solicitudes de Fallecimiento</span>}
            ...
            { state.MostrarEvidencias && 
            <ModalWin open={ state.MostrarEvidencias } center large>
                <ModalWin.Header >
                    <h5 className={ MODAL_TITLE_CLASS }> Evidencia:  </h5>
                    <button type='button' className='delete' onClick={() => {
                        fnCancelarEvidencia()
                    }}></button>
                </ModalWin.Header >
                < ModalWin.Body >
                    <VerActa  DistribuidorID={state.DistribuidorID} Ruta={state.Ruta} fnCancelar={fnCancelarEvidencia}></VerActa>
                </ModalWin.Body >
            </ModalWin>}                             
        </>
    )
...
export default connect(mapStateToProps, mapDispatchToProps)(SolicitudesFallecimiento);

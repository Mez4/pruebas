...
export const AdvertenciaVerificaRefsAvales = (props: FormaNotasTipo) => {
    ...
    const FNVerificarRefsTitular = (ProspectoID: number) => {
        ...
        Funciones.FNupdateVerificaRefsAvales(props.oidc, ProspectoID)
            .then((respuesta: any) => {
                setLoading(false)
    ...
    const FNRechazaRefsTitular = (ProspectoID: number, Nota: string) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateRechazarRefsAvales(props.oidc, { ProspectoID, Nota })
            .then((respuesta: any) => {
                setLoading(false)
                props.cbActualizar(respuesta)

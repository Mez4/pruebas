...
export const AdvertenciaAval = (props: FormaNotasTipo) => {
    ...
    const FNValidaAval = (AvalID: number) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateValidaAval(props.oidc, AvalID)
            .then((respuesta: any) => {
                setLoading(false)
                toast.success('AVAL VALIDADO CORRECTAMENTE', respuesta)
                props.fnCancelaAdvertencia()
                props.fnCerrarInfoAval()
                props.FNGetLocal()
            })
            .catch((error: any) => {
    ...
    const FNRechazaAval = (AvalID: number, Nota: string) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateRechazAval(props.oidc, { AvalID, Nota })
            .then((respuesta: any) => {
                setLoading(false)
                toast.success('AVAL RECHAZADO CORRECTAMENTE', respuesta)
                props.fnCancelaAdvertencia()
                props.fnCerrarInfoAval()
                props.FNGetLocal()
            })
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            {!state.Cargando && <button onClick={() => {
                                if (props.Filtro == 1) {
                                    FNValidaAval(props.AvalID)
                                }
                                else {
                                    FNRechazaAval(props.AvalID, state.Nota)
                                }
                            }}
                            ...
                            <Formik initialValues={{Mensaje: ''}}
                                ...
                                onSubmit={(values: any) => {
                                    ...
                                    if(values.Mensaje === '' && props.Filtro == 2){ 
                                        ...
                                        FNRechazaAval(props.AvalID, `${values.Mensaje} ${state.Nota ? ' - '+state.Nota : ''}`)

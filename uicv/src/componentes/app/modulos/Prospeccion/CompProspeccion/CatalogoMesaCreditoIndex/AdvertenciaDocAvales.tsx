...
import { FaPlus, FaSearch, FaCheckCircle, FaClock, FaCommentDots } from 'react-icons/fa'
...
export const AdvertenciaDocAvales = (props: FormaNotasTipo) => {
    ...
    const GetMensajesFijos = () => {
        setState(s => ({ ...s }))
        Funciones.FnGetMensajesFijos(props.oidc, {StatusProcesoID: 16, TipoDocumentoID: props.tipoDocumentoID })
            .then((respuesta: any) => {
    ...
    const FNAutorizaDocumento = (DocumentoAvalID: number) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateAutorizaDocumentoAval(props.oidc, DocumentoAvalID)
            .then((respuesta: any) => {
                setLoading(false)
                //console.log(respuesta)
                toast.success(respuesta)
                props.fnCancelar() // Cierra el modal de de AdvertenciaDocAvales
                props.fnCancelar2() // Cierra el modal de CFormVerDocumentosAval
                props.FNGetLocal() //Actualizar Datos del modal de CFormVerDocumentosAval
            })
            .catch((error: any) => {
    ...
    const FNRechazaDocumento = (DocumentoAvalID: number, Nota: string) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateRechazaDocumentoAval(props.oidc, { DocumentoAvalID, Nota })
            .then((respuesta: any) => {
                setLoading(false)
                //console.log(respuesta)
                toast.success(respuesta)
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            {!state.Cargando && <button onClick={() => {
                                if (props.Filtro == 1) {
                                    FNAutorizaDocumento(props.DocumentoAvalID)
                                }
                                else {
                                    FNRechazaDocumento(props.DocumentoAvalID, state.Nota)
                                }
                            }}
                                ...
                                onSubmit={(values: any) => {
                                    ...
                                    if(values.Mensaje === '' && props.Filtro == 0){ 
                                        ...
                                        if (props.Filtro == 1) 
                                            ...
                                            FNRechazaDocumento(props.DocumentoAvalID, `${values.Mensaje} ${state.Nota ? ' - '+state.Nota : ''}`)
                                    }

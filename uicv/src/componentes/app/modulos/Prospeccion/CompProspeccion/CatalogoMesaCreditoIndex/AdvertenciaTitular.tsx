...
import { FaBan, FaCommentDots } from 'react-icons/fa'
...
type FormaNotasTipo = {
    ...
    ProspectoID: number,
    fnCancelar(): any,
    fnCancelarVerifica(): any,
    fnCerrarverifica(): any,
...
export const AdvertenciaTitular = (props: FormaNotasTipo) => {
    ...
    const GetMensajesFijos = () => {
        setState(s => ({ ...s }))
        Funciones.FnGetMensajesFijos(props.oidc, {StatusProcesoID: 14, TipoDocumentoID: 0 })
            .then((respuesta: any) => {
    ...
    const FNVerificaTitular = (ProspectoID: number) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateVerificaTitular(props.oidc, ProspectoID)
            .then((respuesta: any) => {
                setLoading(false)
                props.cbActualizar(respuesta)
                toast.success('TITULAR VERIFICADO CORRECTAMENTE', respuesta)
                props.fnCancelar()
                props.fnCancelarVerifica()
                props.fnCerrarverifica()
            })
            .catch((error: any) => {
    ...
    const FNRechazaTitular = (ProspectoID: number, Nota: string) => {
        setState(e=>({...e, Cargando: true}))
        Funciones.FNupdateRechazaTitular(props.oidc, { ProspectoID, Nota })
            .then((respuesta: any) => {
                setLoading(false)
                props.cbActualizar(respuesta)
                toast.success('TITULAR RECHAZADO', respuesta)
                props.fnCancelar()
                props.fnCancelarVerifica()
                props.fnCerrarverifica()
            })
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            {!state.Cargando&& <button onClick={() => {
                                if (props.Filtro == 1) {
                                    FNVerificaTitular(props.ProspectoID)
                                }
                                else {
                                    FNRechazaTitular(props.ProspectoID, state.Nota)
                                }
                            }}
                            ...
                            <Formik initialValues={{Mensaje: ''}}
                                ...
                                onSubmit={(values: any) => {
                                    ...
                                    if(values.Mensaje === '' && props.Filtro == 2){ 
                                        ...
                                        if(props.cancela === true)
                                            fnCancelarProspecto(`${values.Mensaje} ${state.Nota ? ' - '+state.Nota : ''}`)
                                        else
                                            FNRechazaTitular(props.ProspectoID, `${values.Mensaje} ${state.Nota ? ' - '+state.Nota : ''}`)
                            ...
                            <Form>
                                ...
                                <div className="text-center">
                                    <br />
                                    {state.Cargando && <Spinner/>}
                                    {!state.Cargando&& props.Filtro === 2 && props.cancela === true && <button /*onClick={() => { fnEnviarMensaje() }}*/ type="submit" className="ms-1 btn btn-danger waves-effect waves-light"><FaBan size={20} /> CANCELAR PROSPECTO</button>}
                                    {!state.Cargando&& props.Filtro === 2 && props.cancela !== true && <button /*onClick={() => { fnEnviarMensaje() }}*/ type="submit" className="ms-1 btn btn-primary waves-effect waves-light"><FaCommentDots size={20} /> ENVIAR MENSAJE</button>}
                                    {!state.Cargando&& props.Filtro !== 2 && props.cancela !== true && <button /*onClick={() => { fnEnviarMensaje() }}*/ type="submit" className="ms-1 btn btn-success waves-effect waves-light">ACEPTAR</button>}
                                </div>
                            </Form>
                            </Formik>
                        </div>

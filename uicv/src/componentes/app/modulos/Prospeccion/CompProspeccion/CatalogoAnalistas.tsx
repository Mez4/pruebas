...
//import * as FnCatTipoDocumento from '../../catalogos/CompCatalogos/CatalogoAnalistas/Funciones'
...
const CatalogoAnalistas = (props: CatalogosType) => {
    // Controll our mounted state
    let isMounted = React.useRef(true)

    const DatosDefecto = { /*AnalistaID: 0,*/ PersonaID: 0, MesaCreditoID: 0, Activo: false }
    const Datos: any[] = []
    ...
    const FnGetPersonas = () => {
        setState(s => ({ ...s }))
        Funciones.FnGetPersonas(props.oidc)
            .then((respuesta: any) => {
                var sucursales = respuesta.map((valor: any) => {
    ...
    const cbAgregar = (item: any) => {
        setState({ ...state, Datos: [...state.Datos, item], Form: { ...state.Form, Mostrar: false, Datos: { /*AnalistaID: 0,*/ PersonaID: -1, MesaCreditoID: -1, Activo: false } } })
    }

    // /** funcion Callback al actualizar un item */
    const cbActualizar = (item: any) =>
        setState({ ...state, Datos: state.Datos.map(Dato => Dato.AnalistaID === item.AnalistaID ? item : Dato), Form: { ...state.Form, Mostrar: false, Datos: { /*AnalistaID: 0,*/ PersonaID: 0, MesaCreditoID: 0, Activo: false } } })
    ...
    return (
        <div className="row">
            <div className="col-12">
                <Card Title="Catálogo de Analistas">
                    <Card.Body>
                        <Card.Body.Content>
                            ...
                            {!state.Cargando && !state.Error &&
                                <div>
                                    ...
                                    <ModalWin open={state.Form.Mostrar}>
                                        <ModalWin.Header>
                                            <h5 className={MODAL_TITLE_CLASS}>Agregar Analista</h5>
                                        </ModalWin.Header>
                                        <ModalWin.Body>
                                            {<CForm
                                                oidc={props.oidc}

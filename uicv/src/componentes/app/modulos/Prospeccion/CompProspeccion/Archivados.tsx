...
import { FaStickyNote, FaClock, FaFilePdf, FaUserPlus, FaExclamationCircle, FaStore, FaUser, FaArrowDown, FaArrowUp } from 'react-icons/fa'
...
const Archivados = (props: CatalogosType) => {
    ...
    const FNGetLocal = () => {
        ...
        Funciones.FNGetArchivados(props.oidc)
            .then((respuesta: any) => {
                ...
                cell: (props) =>
                    <>
                        <div data-tip data-for={`AP_${props.ProspectoID}`} className="divInDTable text-center radiusSmallDivR">
                            <button onClick={() => fnDesarchivarProspecto(props.ProspectoID)} className="btn btn-outline-default buttonIconInDTable" type={"button"}>
                                <FaArrowUp style={{color:'green'}} size={15} /></button>
                        </div>
                        <ReactTooltip id={`AP_${props.ProspectoID}`} type="info" effect="solid">
                            Desarchivar Prospecto {props.NombreProspecto}
                        </ReactTooltip>
                    </>,
                conditionalCellStyles: activoStyle(props)
            },
    ...
    //DESARCHIVAR PROSPECTO
    const fnDesarchivarProspecto = (ProspectoID: any) => {
        MySwal.fire({
            title: "<strong>DESARCHIVAR PROSPECTO</strong>",
            icon: "question",
            html: (
              <div className="text-center">
                Se desarchivara el prospecto ¿Desea continuar?
              </div>
            ),
            ...
            if (result.isConfirmed) {
                Funciones.FNDesarchivarProspecto(props.oidc, ProspectoID)
            .then((respuesta: any) => {
                cbActualizarM() //Actualiza los Renglones
                toast.success("PROSPECTO DESARCHIVADO CORRECTAMENTE")
            })
            .catch((error: any) => {
        ...
        <>
            ...
            <div className="row ">
                <div className="col-12">
                    <Card Title="Prospectos Archivados">
                        <Card.Body>
                            <Card.Body.Content>
...
export default connect(mapStateToProps, mapDispatchToProps)(Archivados)
...
const Estados = (props: EstadoTipo) => {
    ...
    let opciones = Estado.Datos.map(e => ({ value: e.estadoPaisId, label: e.estadoPaisNombre }))

    // Si queremos un control de una sola linea...
    if (props.unaLinea === true)
        return (
            <CustomSelect2 name={props.name ?? 'estadoPaisId'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Estado'} options={opciones} isMulti={false} mostrarBoton={true} />
        )

    // Si queremos el control con etiqueta
    return (
        <CustomSelect name={props.name ?? 'estadoPaisId'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Estado'} options={opciones} isMulti={false} />
    )
}

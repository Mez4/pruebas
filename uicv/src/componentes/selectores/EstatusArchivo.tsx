...
const EstatusArchivo = (props: TipoProps) => {
    ...
    const FNGetLocal = () => {
        ...
        axios.post(`${GetServerUrl()}Archivo/Archivo/getEstatus`, {}, {
            headers: {
    ...
    let opciones = Estado.Datos.map(e => ({ value: e.EstatusID, label: e.NombreEstatus }))

    // Si queremos un control de una sola linea...
    if (props.unaLinea === true)
        return (
            <CustomSelect2 name={props.name ?? 'EstatusID'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Estatus'} options={opciones} isMulti={false} mostrarBoton={true} />
        )

    // Si queremos el control con etiqueta
    return (
        <CustomSelect name={props.name ?? 'EstatusID'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Estatus'} options={opciones} isMulti={false} />
    )
}

export default EstatusArchivo
 ...
 */
type SucursalFisicaTipo = {
    name?: string,
    unaLinea?: boolean,
    disabled?: boolean,
    oidc: IOidc
}
const SucursalesFisicas = (props: SucursalFisicaTipo) => {
    ...
    if (Estado.Error)
        ...
        placeholder = "Seleccione la Sucursal Fisica"

    console.log('Sucursales Fisicas: ', Estado.Datos)

    // Opciones del control
    let opciones = Estado.Datos.map(e => ({ value: e.SucursalFisicaID, label: e.Nombre, group: e.Estado }))

    // Si queremos un control de una sola linea...
    if (props.unaLinea === true)
        return (
            <CustomSelect2 name={props.name ?? 'SucursalFisicaID'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Sucursal Fisica'} options={opciones ?? []} isMulti={false} mostrarBoton={true} />
        )

    // Si queremos el control con etiqueta
    return (
        <CustomSelect name={props.name ?? 'SucursalFisicaID'} disabled={props.disabled ?? false} addDefault={true} placeholder={placeholder} label={'Sucursal Fisica'} options={opciones ?? []} isMulti={false} />
    )
}

export default SucursalesFisicas
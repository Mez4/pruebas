...
export const FormaDatosGeneralesLaboral = ({ Prefijo, Titulo, SubTitulo, TieneEmpleo, OcupacionID, AsentID }: DireccionTipo) => {
    ...
    InitialValues[`${Prefijo}CalleLaboral`] = ''
    ...
    if (TrabajaDisplay === 'block') {
        ...
        ValSchemaObjecet[`${Prefijo}Antiguedad`] = Yup.string().required().min(3).max(25)
        ...
        ValSchemaObjecet[`${Prefijo}LocalidadLaboral`] = Yup.string().required().min(1).max(50)
        ValSchemaObjecet[`${Prefijo}CalleLaboral`] = Yup.string().required().min(1).max(50)
        ValSchemaObjecet[`${Prefijo}NumeroExteriorLaboral`] = Yup.string().required().min(1).max(10)
    }
    ...
    return {
        ...
        Componente: (cprops: any) => (
            <div>
                <CustomSelect2 name={`${Prefijo}Trabaja`} disabled={false} options={[{ value: true, label: 'Si, tiene empleo.' }, { value: false, label: 'No, no tiene empleo.' }]} addDefault={false} label={'¿Trabaja?'} placeholder={'¿El prospecto tiene empleo actualmente?'} accion={TrabajaSelected} valor={trabaja} />
                <div style={{ display: TrabajaDisplay }}>
                    <CustomFieldText2 {...cprops} label={'Empresa'} datoType='text' name={`${Prefijo}Empresa`} placeholder={'Empresa en que labora el prospecto'} />
                    ...
                    <div className={"row"}>
                        <div className={"col-sm-12 col-md-6"}>
                            <CustomFieldText2 {...cprops} label={'Calle'} datoType='text' name={`${Prefijo}CalleLaboral`} placeholder={'Calle'} />
                        </div>
                        <div className={"col-sm-12 col-md-6"}>
                            <CustomFieldText2 {...cprops} label={'Localidad'} datoType='text' name={`${Prefijo}LocalidadLaboral`} placeholder={'Localidad o ciudad'} />
                        </div>
                    </div>
                    <div className={"row"}>
                        <div className={"col-sm-12 col-md-6"}>
                            <CustomFieldText2 {...cprops} label={'Numero'} datoType='text' name={`${Prefijo}NumeroExteriorLaboral`} placeholder={'Numero Exterior'} />
                        </div>
                        <div className={"col-sm-12 col-md-6"}>
                            <CustomFieldText2 {...cprops} label={'Teléfono'} datoType='text' name={`${Prefijo}TelefonoLaboral`} placeholder={'Teléfono de Trabajo'} />
                        </div>
                    </div>
                </div>

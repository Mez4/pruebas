...
// Iconos
import { FaCheck, FaUserAlt, FaEye, FaMoneyCheck, FaMoneyBillWave } from 'react-icons/fa';
...
import Creditos from '../../selectores/Creditos';
import { DBConfia_Creditos } from '../../../interfaces_db/DBConfia/Creditos';
import { EstatusCredito } from '../../../redux/catalogos/functionesThunk';
...
export const PerfilPersonaParaCapturaDeVales = (props: PerfilPersonaTipo, ui) => {
    ...
    const DatosDefecto = { DistribuidorID: props.Persona.PersonaID, Telefono: props.Persona.TelefonoDomicilio, EstatusId: props.Creditos/* UsuarioIDModifica: 0,*/ }
    ...
    const datosContCredito: any[] = []
    //const datosCredito = { EstatusId: props.Creditos.EstatusID, ClienteId: props.Creditos.ClienteID, }
    ...
    const DatosContCreditos = {
        ClienteID: 0, NombreCompleto: '', ImporteTotal: 0, SaldoActual: 0, SaldoAtrasado: 0, DiasAtraso: 0, FechaHoraUltimoPago: '', Capital: 0
    }
    ...
    const [state, setState] = React.useState({
        ...
        {
            Mostrar: false,
            datosContCreditos: DatosContCreditos,
            Id: undefined
        },
        FormContrasena:
        {
    ...
    console.log('Estatus:', props.Creditos);
    var credit, credit0, credit2, credit3;
    if (props.Persona.PersonaID) {
        if (props.Creditos?.EstatusID === 'A') {
            if (props.Creditos.SaldoAtrasado > 0) {
        ...
        } else if (props.Creditos?.EstatusID === 'P') {
            credit2 = 'CRÉDITO PENDIENTE';
        }
        else {
            credit3 = 'SIN CRÉDITOS ACTIVOS'
        }
        ...
        <div className="col-12 text-center">
            ...
            {!props.Documentos ?
                ...
                <>
                    ...
                    <a href="#" style={{/* marginLeft: '15em',*/ padding: '0' }} className={"btn btn-link waves-effect"} onClick={(e) => changeImage(e)}><FaEye /> <span>{showFirma ? 'Foto' : 'Firma'}</span></a>
                    <div style={{ width: '100%', height: '150px', backgroundColor: 'white', paddingTop: '3%' }}>
            ...
            <p className={'mt-2 mb-0 fw-bold'}>
                {/*    {props.ui.PermisosProductos?.find(p => p.PermisoID == 112)  &&
            ...
            <nav style={{ color: '#28a745', fontWeight: 'bold' }}>{credit} <div className="text-danger">{credit0}</div></nav>
            <nav style={{ color: '#ffc107', fontWeight: 'bold' }}>{credit2}</nav>
            <nav style={{ color: 'grey', fontWeight: 'bold' }}>{credit3}</nav>

            {props.Persona.UsuarioID == null && <p className='btn btn-link mt-2 mb-0 fw-bold'>Usuario no asignado</p>}
                            ...
                            <table style={{ width: '100%', tableLayout: 'fixed' }}>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{moment(props.Persona.FechaNacimiento).format('DD-MM-YYYY')}</td>
                                    </tr>
                                    <tr>
            ...
            <Tabs TabSelecionado={'ubicacion'} Justified={true} Kind={Tabs.TabsKind.CUSTOM}>
                ...
                <Tabs.Tab Titulo={<><MdOutlineAttachMoney size={20} color='green' /> <br /> CRÉDITOS</>} Identificador={'contcreditos'}>
                    <Acordion TabSelecionado={''}>
                        <div>
                            {props.ContCreditos.map((d, dId) =>
                                <Acordion.Tab key={'job_' + dId} Identificador={'job_' + dId} Titulo={`Crédito [${d.CreditoID}] de ${d.NombreCompleto}`}>
                                    <div className="text-start">
                                        <table style={{ width: '100%', tableLayout: 'fixed' }}>
                                            <tbody>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>N°:</td>
                                                    <td>{d.ClienteID}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Cliente:</td>
                                        ...
                                        <table style={{ width: '100%', tableLayout: 'fixed' }}>
                                            <tbody>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Importe Total:</td>
                                                    <td>{formatter.format(d.ImporteTotal > 0 ? d.ImporteTotal : 0)}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Saldo:</td>
                                                    <td>{formatter.format(d.SaldoActual > 0 ? d.SaldoActual : 0)}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Saldo Atrasado:</td>
                                                    <td>{formatter.format(d.SaldoAtrasado)}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Días Atraso:</td>
                                                    <td>{d.DiasAtraso}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Fecha Último Pago:</td>
                                                    <td>{moment(d.FechaHoraUltimoPago).format('DD-MM-YYYY')}</td>
                                                </tr>
                                                <tr>
                                                    <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Capital:</td>
                                                    <td>{formatter.format(d.Capital)}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>

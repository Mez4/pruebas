...
export const PerfilPersonaParaGlobal = (props: PerfilPersonaTipo, ui) => {
        ...
        <div className="col-12 text-center">
            ...
            {!props.Documentos ?
                ...
                <>
                    ...
                    {props.Persona.DistribuidorIDVR && <p className={"mb-1 fw-bold"} style={{ color: props.Persona.BuroInternoColor }}>ID Socia VR: {props.Persona.DistribuidorIDVR}</p>}
            ...
            <div className={'mt-2 mb-0 fw-bold'}>
                <div className={`d-flex justify-content-center col-sm-3 col-md-6 ${props.ui.PermisosProductos?.find(p => p.PermisoID == 112) ? 'col-lg-10' : 'col-lg-12'}   `}>
                    ...
                    {props.ui.PermisosProductos?.find(p => p.PermisoID == 112) /*props.Editar */ &&
                        <>
                            <div className="d-flex justify-content-end">
                                <FaPencilAlt className='' size={20} style={{ color: 'rgb(76, 175, 80)', cursor: 'pointer', position: 'relative', left: 45 }} onClick={() => setState({ ...state, FormNombre: { Mostrar: true, DatosNombre: DatosNombre, Id: undefined } })}></FaPencilAlt>
                            </div>
                        </>
                    }
                        ...
                        <div className="text-start">
                            <table style={{ width: '100%', tableLayout: 'fixed' }}>
                                <tbody>
                                    ...
                                    <tr>
                                        {/* { name: 'Fecha Aplicacion', selector: 'FechaAplicacion', center: true, maxWidth: '130px', wrap: true, cell: (props) => <span>{moment.parseZone(props.FechaAplicacion).format("DD/MM/YYYY")}</span> }, */}

                                        {/* <td> cell: (props) => <span>{moment.parseZone(props.FechaAplicacion).format("DD/MM/YYYY")}</span></td> */}
                                        <td style={{ display: 'flex', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', }}>F.Nacimiento:</td>
                                        <td>{moment.parseZone(props.Persona.FechaNacimiento).format("DD/MM/YYYY")}</td>
                        ...
                        <div>
                            {props.Editar && props.ui.PermisosProductos?.find(p => p.PermisoID == 470) &&
                                <div style={{ position: 'absolute', right: '15px', zIndex: 10, cursor: 'pointer' }} title={'Agregar dirección'}
                                    onClick={() => setState({ ...state, FormDireccion: { Mostrar: true, datosDirecciones: DatosDireccion, Id: undefined, } })} /*onClick={() => alert("DOMO !")}*/>
                                    <FcPlus size={25} style={{ position: 'fixed', left: 175, zIndex: 100 }} />
                                </div>
                            }

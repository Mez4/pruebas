...
namespace ConfiaWebApi.Controllers.Creditos
{
    ...
    public class NivelesProdController : ControllerBase
    {
        private DBConfiaContext DBContext;
        public NivelesProdController(DBConfiaContext _DBContext)
        {
        ...
        public async Task<IActionResult> GetByDirector(PeticionesRest.Creditos.Producto.Get parData)
        {
            try
            {
                var res = await DBContext.database.QueryAsync<ProductosVW>("WHERE (PersonaResponsableID = @0 OR @0 = 0) AND ProductoID IN (@1)", parData.DirectorID, parData.AccesoProductos).ToArrayAsync();//.OrderBy(x => x.DistribuidorID)
                await DBContext.Destroy();
                return Ok(res);
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> Obtener()
        {
            ...
            try
            {
                ...
                transaccion = true;
                var Niveles = await DBContext.database.FetchAsync<NivelesProductos_VW>();
        ...
        [Authorize]
        [Code.TProteccionProducto]
        public async Task<IActionResult> obtenerProducto(ConfiaWebApi.PeticionesRest.Creditos.Niveles.Get parData)
        {
            var transaccion = false;
            /*   var Distribuidor = await DBContext.database.SingleByIdAsync<NivelesProductos>((parData.DistribuidorNivelID, parData.ProductoID));
              var Producto = await DBContext.database.SingleByIdAsync<Productos>(Distribuidor.ProductoID); */
            try
            {
                DBContext.database.BeginTransaction();
                transaccion = true;
        ...
        /*Belicxs somos Belicxs morimos*/
        [HttpPost]
        [Route("insertar")]
        [Code.TProteccionProducto]
        [Authorize]
        public async Task<IActionResult> insertar(ConfiaWebApi.PeticionesRest.Creditos.NivelesProducto.Agregar parData)
        {
            var transaccion = false;
            try
            {
                var registro = new DBContext.DBConfia.Distribuidores.NivelesProductos();
                {
                    registro.ProductoID = parData.ProductoID;
                    ...
                    registro.IncrementoQuincena = parData.IncrementoQuincena;
                    registro.IncrementoQuincenaCalidadBaja = parData.IncrementoQuincenaCalidadBaja;
        ...
        [Code.TProteccionProducto]
        [Authorize]
        public async Task<IActionResult> update(ConfiaWebApi.PeticionesRest.Creditos.NivelesProducto.Update parData)
        {
            var transaccion = false;
            try
            {
                var registro = await DBContext.database.QueryAsync<NivelesProductos>("WHERE ProductoID=@0 AND DistribuidorNivelID=@1", parData.ProductoID, parData.DistribuidorNivelID).SingleOrDefaultAsync();
                if (registro != null)
                {
                    registro.ProductoID = parData.ProductoID;
                    registro.DistribuidorNivelID = parData.DistribuidorNivelID;
        ...
        [Route("actualizar")]
        [Code.TProteccionProducto]
        public async Task<IActionResult> actualizar(ConfiaWebApi.PeticionesRest.Creditos.NivelesProducto.Update parData)
        {
            var transaccion = false;
            try
            {
                var SolicitudEncontrada = await DBContext.database.SingleByIdAsync<NivelesProductos>(parData.ProductoID);
                if (SolicitudEncontrada != null)
                {
                    SolicitudEncontrada.ProductoID = parData.ProductoID;
                    DBContext.database.BeginTransaction();
                    transaccion = true;

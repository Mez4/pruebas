...
namespace ConfiaWebApi.Controllers.AppProspeccion
{
    ...
    public class AppAvalesController : ControllerBase
    {
        private DBConfiaContext ConexionBD;

        private IConfiguration Configuracion;

        public AppAvalesController(IConfiguration _Configuration, DBConfiaContext _DBContext)
        {
        ...
        public async Task<IActionResult> GetByProspecto(ConfiaWebApi.PeticionesRest.Prospeccion.Avales.getByProspecto parData)
        {
            try
            {
                var res = await ConexionBD.database.QueryAsync<AvalesaApp_VW>("WHERE ProspectoID = @ProspectoID", parData).ToArrayAsync();
       ...
       // [Code.TProteccionProducto]
        public async Task<IActionResult> getByProspectoAval(ConfiaWebApi.PeticionesRest.Prospeccion.Avales.getByProspecto parData)
        {
            try
            {
                ...
                var avales = await ConexionBD.database.QueryAsync<AvalesaApp_VW>("WHERE avalID=@0", parData.ProspectoID).FirstOrDefaultAsync();
                await ConexionBD.Destroy();

...
#pragma warning disable IDE0037  // Desactivar la advertencia
    ...
    public class AppLoginController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("iniciarSesion")]
        public async Task<IActionResult> getToken(ConfiaWebApi.PeticionesRest.AppLogin.AppLogin.LoginAutenticacion pardata)
        {
            try
            {
                ...
                var res = await GetTokenLclAsync(aux, pardata.password);
                ...
                if (res.IsSuccessStatusCode)
                {
                    ...
                    var usuario = await DBContext.database.QueryAsync<DBContext.DBConfia.Seguridad.UsuariosApp_VW>("WHERE Usuario = @0", aux).SingleOrDefaultAsync();

                    //Convertir colores a configuraciones
                    //TODO : REDIRIGIR CONSULTA A EMPRESAID
                    var colores = await DBContext.database.QueryAsync<DBContext.DBConfia.General.ColoresProductoApp_VW>("WHERE EmpresaId = @0", usuario.empresaId).ToArrayAsync();

                    //Crear Diccionario de colores
                    Dictionary<string, Int64> coloresConfig = new Dictionary<string, Int64>();

                    if (colores == null || colores.Length == 0)
                    {
                        coloresConfig.Add("TipoEmpresaId", 1);
                    }
                    else
                    {
                        coloresConfig.Add("TipoEmpresaId", (int)colores[0].TipoEmpresaId);
                    }
        ...
        public async Task<IActionResult> GetTokenDV(PeticionesRest.AppLogin.AppLogin.LoginDV pardata)
        {
            try
            {
                ...
                var tipoEmpresa = await DBContext.database.QueryAsync<Empresas>("WHERE EmpresaId = @0 ", usuario.empresaId).SingleOrDefaultAsync();

                if (usuario == null || tipoEmpresa == null || tipoEmpresa.TipoEmpresaID != 1)
                {
                    await DBContext.Destroy();
                    return BadRequest(new
                    {
        ...
        public async Task<IActionResult> RecuperarContrasena(PeticionesRest.AppLogin.AppLogin.CambioContraPromotor pardata)
        {
            try
            {
                ...
                var passEncriptada = BCrypt.Net.BCrypt.HashPassword(pardata.Contrasena);
        ...
        public async Task<IActionResult> ValidarSocia(PeticionesRest.AppLogin.AppLogin.ValidacionDv pardata)
        {
            try
            {
                ...
                //Obtener token
                var res = await GetTokenLclAsync(usuarioWithUseranme.Usuario, pardata.Password);
                ...
                var dat = await DBContext.database.QueryAsync<Usuarios_Personas>("WHERE PersonaID = @0 AND UsuarioID = @1", Distribuidor.PersonaID, usuario.UsuarioID).SingleOrDefaultAsync();
